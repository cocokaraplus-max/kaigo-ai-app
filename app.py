...
if not device_id:
    ...
    cookie_manager.set("device_id", device_id, key="dev_id_v36")

# 端末ブロックチェック
if device_id:
    try:
        res_block = supabase.table("blocked_devices").select("*").eq("device_id", device_id).eq("is_active", True).execute()
        if res_block.data: st.error("🚫 この端末はアクセス制限されています。"); st.stop()
    except: pass
            ...
            if f_in and n_in:
                try:
                    ...
                    if res_n and res_n.data:
                        st.error(f"🚫 {n_in} 様はアクセス制限されています。")
                        st.stop()
                except: pass # エラー時は一旦スルー
...
pages = {
    ...
    "admin_menu": views.render_admin_menu
}
pages[st.session_state["page"]](supabase, cookie_manager, f_code, my_name) if st.session_state["page"] != "admin_menu" else views.render_admin_menu(supabase, cookie_manager, f_code, my_name, device_id)