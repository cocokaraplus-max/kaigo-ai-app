...
with tab1:
    ...
    with col1:
        ...
        st.subheader("📸 写真を追加")
        use_camera = st.checkbox("カメラを起動")
        if use_camera:
            img_file = st.camera_input("撮影")
        else:
            # ラベル自体を日本語にすることで「Drag and drop」を回避
            img_file = st.file_uploader("写真を選択してください", type=['jpg', 'png', 'jpeg'], label_visibility="visible")
        
        if img_file:
            st.image(img_file, caption="選択済み", width=250)
    ...
    with col2:
        ...
        final_content = st.text_area("記録内容", value=st.session_state.get("edit_content", ""), height=300)
        ...
        if st.button("💾 クラウドに保存する"):
            if user_name:
                ...
                msg_placeholder.info("⏳ クラウドに保存しています。そのままお待ちください...")
                ...
                try:
                    ...
                    record_data = {
                        "user_name": user_name,
                        "content": final_content if final_content else "（画像保存）",
                        "image_url": image_url, 
                        "created_at": target_date.isoformat()
                    }
                    supabase.table("records").insert(record_data).execute()
                    
                    msg_placeholder.success(f"✅ 【{user_name}様】の記録を正常に保存しました！")
                    time.sleep(2.0)
                    st.session_state["edit_content"] = ""
                    st.session_state["user_name_val"] = ""
                    st.rerun()
                except Exception as e:
                    msg_placeholder.error(f"❌ 保存できませんでした。エラー: {e}")
            else:
