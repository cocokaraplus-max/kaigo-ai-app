...
# --- 🎨 カスタムCSS（視認性向上 & ボタンデザイン） ---
st.markdown("""
    <style>
    /* 管理者メニューのタイトル */
    .admin-title {
        font-size: 22px; font-weight: bold; color: #ff4b4b !important;
        padding-bottom: 10px; border-bottom: 2px solid #ff4b4b; margin-bottom: 20px;
        display: block;
    }
    /* タブの文字色を濃いグレーに固定（ライトモード対応） */
...
# ==========================================
    ...
    with tab4:
        new_pw = st.text_input("新しいパスワード", type="password"); confirm_pw = st.text_input("確認用", type="password")
    ...
    st.divider()
    if st.button("◀ TOPに戻る", key="adm_down"): st.session_state["page"] = "top"; st.rerun()
...
# ==========================================
        ...
        with st.spinner("AI整理中..."):
            try:
                ...
                for _ in range(10):
                    ...
                    time.sleep(1.5)
                # 申し送り口調
                response = model.generate_content([f_up, "介護職員が仲間に送る自然な口調で、話した内容のみを正確に整理してください。"])
    ...
    st.divider()
    if st.button("◀ TOPに戻る", key="inp_down"): st.session_state["page"] = "top"; st.rerun()
    ...
    if sel != "---":
        ...
        if st.button("📈 1ヶ月モニタリング作成", use_container_width=True):
            ...
            if res.data:
                ...
                model = genai.GenerativeModel("models/gemini-2.5-flash")
                resp = model.generate_content(f"ケアマネジャー報告用の200字モニタリング。支援内容重視。\n\n{all_txt}")
        ...
        if st.session_state["show_history_list"]:
            ...
            for r in res.data:
                with st.expander(f"📅 {r['created_at'][:16].replace('T',' ')} - 記: {r.get('staff_name','--')}"):
                    st.write(r['content'])
    st.divider()
    if st.button("◀ TOPに戻る", key="his_down"): st.session_state["page"] = "top"; st.rerun()