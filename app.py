...
if not device_id:
    ...
    cookie_manager.set("device_id", device_id, key="dev_save")
    ...
    with st.container(border=True):
        ...
        if st.button("利用を開始する", use_container_width=True):
            if f_in and n_in:
                ...
                if res_n.data: st.error("🚫 アクセス制限中です。")
                else:
...
if st.session_state["page"] == "top": views.render_top(supabase, cookie_manager, f_code, my_name)
elif st.session_state["page"] == "input": views.render_input(supabase, f_code, my_name)
elif st.session_state["page"] == "history": views.render_history(supabase, f_code, my_name)
elif st.session_state["page"] == "daily_view": views.render_daily_view(supabase, f_code, my_name)
elif st.session_state["page"] == "admin_menu": views.render_admin_menu(supabase, cookie_manager, f_code, my_name, device_id)