...
const CACHE_VERSION = 'tasukaru-v20';
...
self.addEventListener('push', event => {
  ...
  const title = data.title || 'TASUKARUトーク';
  const body  = data.body  || '新着メッセージがあります';
  ...
  event.waitUntil(
    Promise.all([
      ...
      // ★ ホーム画面アイコンにバッジ数を表示
      navigator.setAppBadge ? navigator.setAppBadge(badge) : Promise.resolve(),
    ])
  );
});
  ...
  try {
    ...
    if (request.method === 'POST') {
      try {
        ...
        return new Response(JSON.stringify({ status: 'error', message: 'オフライン保存に失敗しました' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
    }
