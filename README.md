...
### 追記: AI生成文自体の相対表現も抑止（reldate-normalize-v1 / DEV検証で判明）
- 入力（記録本文）の正規化に加え、`api_generate_monitoring` と `_auto_generate_monitoring` の `BASE_PROMPT` に「今日・本日・昨日・一昨日・明日・先月・先々月・今月・来月などの相対表現は文中で使わず、◯月◯日／◯月と具体的に書く」ルールを追記。DEV検証で、記録本文由来の「本日」は出力に残らない一方、AIが地の文でまれに「今月」等を使うことがあったため（例:「今月の様子ですが」）、AIの生成文側もこのルールで抑止する。
- DEV検証（岡田 清 2026-04・本日6件を含む91記録・まとめて1本）: 記録由来の相対表現は出力ゼロを確認。
