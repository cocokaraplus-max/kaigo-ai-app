...
---
## 【未着手】「今日の予定」の帯をTOPページだけに出す  <!-- cal-today-bar-toponly-v1 -->

**2026-08-19 に現場から出た要望。まだ手を付けていない。次のチャットで対応すること。**

### 経緯
`cal-today-bar-v1` では「今日の予定」の細い帯を**全画面共通**（`base.html`）に出した。
そうした理由は、実装中にユーザーから **「必ずTOPページを経由する仕様にしないと意味がない」**
という指摘があり、TOPを開かなくても目に入るようにしたほうがよいと判断したため。

その後 **現場に出したところ「今日の予定はTOPページに表示するだけでいい」** という声が返ってきた。
全画面に出すのは過剰だった、ということ。**現場の声を優先してTOP限定に戻す。**

### やること（`templates/base.html` の1か所だけ）
`cal-today-bar-v1` のスクリプト内の `load()` の冒頭に、TOP以外なら何もしない1行を足す。
...
これだけで、TOP以外では帯のDOMも出ず、`/api/my_upcoming_events` も叩かなくなる。
...
| 「今日の予定」の帯（`cal-today-bar-v1`） | **TOPだけに変更** | 現場の要望。見た目の話なのでTOPで足りる |

「TOP限定にする」という言葉に引きずられて通知側まで触ると、**アプリを閉じていても鳴るという
この機能の一番の価値が消える。** 触るのは `load()` の1か所だけでよい。

### 検証のしかた
1. DEVで `/top` を開き、今日の予定があるときに帯が出ること（従来どおり）
2. `/board` `/vitals` `/calendar` を開き、**帯が出ないこと**
3. ブラウザの開発者ツールのネットワークで、TOP以外では `my_upcoming_events` が呼ばれないこと
4. 通知の予約が生きていることの確認は `cal-notify-v1` の節の方法（`window.Capacitor` を偽物に
   差し替えて `CAL_NOTIFY.sync()` の結果をのぞく）を使う。**TOP以外の画面で試すこと**

---

## 作業の受け渡しについて（2026-08-19 時点の状況）  <!-- handoff-note-2026-08-19 -->

- **Macを新しくしたのが原因で、AI側からMacのファイルを直接読み書きできなくなった。**
  `device_bash` / `device_stage_files` / `device_commit_files` がすべて
  「デバイスがブリッジに接続されていません」で失敗する。
  チャットを開始した時点のMac（＝古いMac）にセッションが紐づいているためと思われる。
- **対処：新しいMacで新しいチャットを始めれば、そのMacに紐づいて直接書き込みが復活する見込み。**
  新しいチャットのAIには、まず `README.md` と `SESSION_67_HANDOFF.md` を読ませること。
- 直接書き込みが使えない間にうまくいった代替手段は
  `env-note-2026-08-19` の節にまとめてある（`python3 -c` の1行コマンド／VSCodeへの直接貼り付け）。

---
## 【重要】新しいチャットを始めるAIへ  <!-- session-67-entrypoint -->

**まず `SESSION_67_HANDOFF.md` を読むこと。** 2026-08-18 時点の作業状況・次のタスク・
...
### ファイルの受け渡し
- Mac側のシェル（`device_bash`）が起動しないことがある。その場合：
  - **AIが読みたいファイルは、ダウンロードフォルダへコピーするコマンドを提示する。**
    ユーザーがそれをチャットに添付してくれる（この段取りは合意済み）。
    例：`cp "/Users/ZIMAX 1/dev/kaigo-ai-app/app.py" ~/Downloads/`
  - **修正したファイルは `SendUserFile` → `device_commit_files` で直接書き戻せる。**
    そのあとの `git add / commit / push` はユーザーに実行してもらう（コマンドを丸ごと渡す）。
...
### 次の最優先タスク
**積み残しは2つ。どちらも本文に手順まで書いてある。**
1. **「今日の予定」の帯をTOPページだけに出す**（現場の要望・未着手）。
   `cal-today-bar-toponly-v1` の節のとおりに直す。
   **通知の予約は base.html のまま動かさないこと**（動かすとTOPを経由しないと鳴らなくなる）。
