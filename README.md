...
## 利用者モードを「いったん保留」できるようにする 2026-08-24  <!-- self-eval-pause-v1 -->

### 何が困っていたか

利用者モード（タブレットを渡すモード）に入ると、**ロック画面へ行く道が2つしか無かった。**

1. 利用者が最後まで答え終わる
2. 3分さわらずに放置する

途中で職員が呼ばれる、その場でやめる、は**ふつうに起きる**。
それなのに他のメニューへ移れず、**3分待つしかなかった。**

### 直し方

- **利用者モードの画面の右上に、小さく「職員の方へ」**（`self_eval_run.html`）
- 押すと `POST /api/self-eval/pause` → **ロック画面**へ
- ロック画面は保留中だと文言が変わる：「いったん おやすみしています」
- **4桁を入れれば職員の画面に戻り、他のメニューへ自由に行ける**

### ★漏れないようにしている理由

**「職員の方へ」を押しても、行き先はロック画面まで。** そこから先へ出るには
**これまでどおり4桁が要る。** 利用者名が並ぶ画面へは、4桁なしでは一歩も進めない。
`_kiosk_guard` は一切ゆるめていない（許可URLに足したのは `pause` と `resume` の2本だけ）。

★**「回答に もどる」だけは4桁を要らなくした。**
　戻る先は**その方自身の回答画面**だけで、他人の情報は出ない。
　利用者が誤って「職員の方へ」を押したときに、職員を呼ばずに自分で戻れるほうがよい。

★**`resume` は「保留中」のときだけ通す。**
　回答が終わってロックされている状態から戻せると、
　**確定待ちの評価に利用者が上書きできてしまう。**

### 再開のしかた

答えは**1問ごとに保存されている**ので、保留して他の仕事をしたあと、
一覧の「タブレットを渡す」を押せば**答えていない質問から再開**する
（`self_eval_run.html` の `load()` が、点数の入っていない最初の質問へ飛ぶ）。
