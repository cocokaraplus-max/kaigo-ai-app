...
---
# 【設計・未着手】利用者セルフ評価（タブレット）  <!-- self-eval-design-2026-08-20 -->

現場からの依頼：**評価のときに、利用者本人にタブレットで答えてもらう。**
2026-08-20 にユーザーと設計を詰めた。**まだ1行も実装していない。** ここに書いてあるとおりに作ること。
...
3. **利用者セルフ評価（タブレット）の実装。** ★大きな新機能★
   設計は `self-eval-design-2026-08-20` の節に全部書いてある（2026-08-20 にユーザーと詰めた）。
   **未着手。まだ1行も書いていない。** 第1段から順に作ること。
...
- **利用者セルフ評価を作るときは、必ず `before_request` のサーバ側ロックを入れる。**
  画面を隠すだけでは、URLを直打ちされて他の利用者の記録に到達される。
  詳細は `self-eval-design-2026-08-20` の「5.」。
