...
### 0. 渡すものには必ずラベルを付ける  <!-- readme-20260910 -->

AI側がコマンドやURLを渡すときは、**どこへ貼るものか**を毎回頭に書く。

| ラベル | 貼る先 |
|---|---|
| ［ターミナルで実行］ | VSCodeのターミナル |
| ［ブラウザで開く］ | Chromeのアドレスバー |
| ［SupabaseのSQL・DEV］ | DEV（`otjevnmo…`）のSQLエディタ |
| ［SupabaseのSQL・本番］ | 本番（`abvglnkw…`）のSQLエディタ |

★実際に取り違えが起きている。URLをターミナルに貼って
`No such file or directory`、ターミナルのコマンドをアドレスバーに貼って
Google検索になった。**書き分けるのは渡す側の仕事。**

★SQLは「DEVか本番か」を**聞かれる前に**書く。DEVと本番はSupabaseが別なので、
片方に流しただけでは足りないことがある。
...
## Androidで取っ手のタップが効かない件（`drawer-tap-v1` → `drawer-tap-v2`）

### 症状

iPhoneは取っ手をタップしても開くが、**Androidはスライドしないと開かない**。

### ★最初の見立ては【外れ】だった（`drawer-tap-v1`・取り消し済み）

「6px 動いただけで引き出しモードに入る。Androidは指のブレをそのまま出すので、
静止したつもりのタップでも超えてしまう」と読んで、しきい値を上げた。

**実機で測ったら、指のずれは 0px だった。** ブレは関係なかった。
`drawer-tap-v1` はまるごと取り消してある。
★**残しておくと「あれが効いている」と後の人が誤解する。外れた対策は消すこと。**

### 本当の原因（実機の記録 ＋ DEVでの再現で確定）

Pixel 9 Pro / Android 17 / **LINEのアプリ内ブラウザ**（`Line/26.13.0/IAB`、WebView）で
診断ページ（`static/tap.html`）を叩いて取った記録：

```
A     0ms  pointerdown   種類=touch  捕捉=できた
A    72ms  pointerup     ずれ dx=0 dy=0  最大 0px  move 0回
A    74ms  判定 → 開く（タップとして扱われた）   ← 取っ手は正しく動いている
A    96ms  click が来た                          ← ★これが犯人
```

**取っ手の判定は最初から正しかった。問題は開いた【あと】。**

```
72ms  open() が走る
        mask.classList.add('on')     → マスクが全画面を覆う(pointer-events:auto)
        handle.classList.add('hide') → 取っ手が pointer-events:none になる
      そして  mask.onclick = close;
96ms  ブラウザが【合成click】を同じ座標へ送る
        取っ手は無反応 → 下にいるマスクが受け取る → close()
```

**開いた瞬間に、自分で閉じていた。** だから「何も起きない」ように見える。

機種差の理由：
**iOS Safari は `pointerdown` で `preventDefault()` すると合成clickを出さない。
Chromium（Android・WebView）は出す。** スライドのときは指が動くぶん
ブラウザが合成clickを抑えるので起きない。

DEVでの再現（2026-08-27）：開いた直後に取っ手の座標へ click を送ると、
当たる相手は `appDrawerMask`、その場で閉じた。★推測ではなく実測。

### 直した内容（`drawer-tap-v2`）

マスクは「開いた直後の350msだけ」閉じる指示を受け付けない。

```js
var openedAt = 0;
function open() { openedAt = Date.now(); ... }
...
★**取っ手側はいじらない。** 合成clickの出方はブラウザごとに違うので、
　そこを押さえにいくと同じことを繰り返す。**閉じる側で無視する**ほうが、
　どのブラウザでも同じに効く。

★人がマスクを押して閉じる操作は妨げない。開いて350ms以内にわざと押す操作は
　現実には無く、万一そうなっても、もう一度押せば閉じる。

デプロイ前にDEVの実画面で3通り確認：
24ms後の合成click→開いたまま／96ms後→開いたまま／400ms後に人が押す→閉じた。

### ★診断ページ `static/tap.html`

同じことがまた起きたときのために残してある。
端末で開いて取っ手を1回タップすると、
**どのイベントが何ミリ秒後に来たか・指が何px動いたか・pointer捕捉ができたか**が
画面に出る。開発者ツールもケーブルも要らない。

- A ＝ 本物と同じ作り（捕捉・preventDefault・長押しタイマー）
- B ＝ 素の判定。**Bが効いてAが効かなければ、原因はAの作り込み側**

★届け方の教訓：
　`claude.ai` のリンクはAndroidで開くとClaudeアプリが起動してページが出ない。
　ファイル名の `_`（アンダースコア）はLINEで送ると化ける。
　→ **アプリの `static/` に置き、名前は `tap.html`。QRコードで渡すのが確実。**
...
5. **★手元で再現できない不具合は、推測を重ねずに【実機から事実を取る道具】を作る。**
   Androidのタップは2回とも見当違いの直しをした。
   3回目に診断ページ（`static/tap.html`）を作って記録を取ったら、
   1回で原因が確定した。**2回外したら、直し方ではなく調べ方を変えること。**
6. **外れた対策は消す。** `drawer-tap-v1` は残せば「あれが効いている」と
   後の人が誤解する。原因が分かった時点で、まるごと取り消した。
7. **職員が見ている環境は Chrome とは限らない。**
   UAを見て初めて `Line/26.13.0/IAB`（LINEのアプリ内ブラウザ・WebView）と分かった。
   「Androidだから」ではなく「その環境だから」を先に疑うこと。

---

# 本番リリース 2026-08-27（マージ `d181d27`）

`tasukaru-dev` → `tasukaru` に `--no-ff` でマージ。**12ファイル・4,263行追加・46行削除。**

★**戻すときは `git revert -m 1 d181d27` → push。3〜4分で元に戻る。**
　慌てて個別に直そうとしないこと。
...
| | |
|---|---|
| 画面の案内役 | **43画面**（右上の「?」／欄の横の「?」／いまの状況12種類） |
| 評価の入口分割 | `/assessment2`。★**旧 `/assessment` はそのまま残してある** |
| カレンダー | マスの上半分＝勤務予定／下半分＝予定 |
| Androidの取っ手 | タップで開くように（`drawer-tap-v2`） |
| `/case_records`・`/history` | 500を解消（`/monitoring` へ転送） |
| 点検スクリプト | `check_guide.py` |
| 診断ページ | `static/tap.html` |

★**`templates/assessment.html` は1行も変えていない。** 職員が今まで使っている評価画面は無傷。
　新しい入口は `/assessment2` という**別のURL**なので、今までどおり使う限り触れない。

## ★出す前に必ずやったこと（次も同じ順で）

1. **本番SupabaseでDDLの実在確認**（`information_schema.columns`）
   - READMEに「DDLを流したつもりが効いておらず確認クエリ0件」という事故の記録があるため
   - ★**さらに「そのSQLを流したのは本当に本番か」まで確かめた。**
     DEVで流しても同じ結果が返るので、列の数だけでは見分けがつかない。
     利用者数（本番86人／DEV53人）と `facility_code` を同時に数えて確定させた。
2. `git log --oneline tasukaru..tasukaru-dev` で何が出るかを一覧
3. `git diff --stat` で**削除された46行**を洗い出し、1件ずつ何かを確認
   （追加は事故になりにくい。**危ないのは消えた行のほう**）
4. 戻し方を**先に**決めてから push

## 出したあとに見たもの（この順）

```
① TOP・記録入力・バイタル・評価・モニタリング・カレンダー・掲示板 … 全部200
   ★base.html は全画面の土台なので、まずここ
② 右上の「?」が出る／カードが開く
③ 「いまの状況」が本番の実データで出る
④ /case_records が 500 → 200（/monitoring へ転送）
⑤ 旧・評価画面が無傷か（evalFillForm・_axisRestoreNew・_fillOk の実在をHTMLで確認）
⑥ 取っ手のタップ（24ms後の合成clickでも開いたまま）
```

★本番では**読むだけ**（GETとDOM読み取りのみ）。確認のためにデータを1件も作らない・変えない。
　画面を触った場合は、必ず元の状態に戻してから終わる。

## 本番の実データで出た「いまの状況」（2026-08-27 時点）

```
記録入力     今日来られた14人ぶん、記録はそろっています。       ← 緑
利用者情報   介護区分が空の方が 2人（横地美恵子、嶺村竹男）
評価         2026-08 の評価は 34人ぶん。あと 44人 残っています。
体重・体力   今月の体重測定は 51人ぶん。あと 27人 残っています。
モニタリング 2026-08：確定 34人、下書きのまま 36人。
```

★これはもう「案内」ではなく**業務の見える化**になっている。
　「下書きのまま36人」は、言われなければ気づきにくい数字。

★介護区分が空は、以前の調査では7人だった。
　**利用中の方だけを数える**（`is_discontinued` を除く）ようにしたので2人に減った。
　この2人を埋めれば、評価の目標欄が正しく出るようになる。

---

## 2026-09-01 送迎の作り替え（本番 `110edd4` まで）

★詳しい引き継ぎは **`引き継ぎ_2026-09-01.md`**。次のチャットはまずそれを読むこと。

### きっかけ

「送迎表で運行した後に記録表に反映されていない」（HIROさん）。
9/1 の本番を調べると、その日の立ち寄り 27件のうち **到着の打刻が 0件**だった。
そこから、運行画面をひととおり作り替えることになった。

### 記録が残らなかった原因（3つ見つけた）

1. **押しても何も起きなかった**
   `fetch` が投げると、赤い帯を出す行まで届かず、運転手には「押したのに何も起きない」
   だけが残っていた。**Chromium で3通り測って確かめた**（圏外／ログイン切れ／エラー）。
   → `soge-tap-retry-v1`。送り口を1本にして、例外を外に出さない。
     圏外の打刻は端末に貯めて、電波が戻ったら**押した時刻のまま**送り直す。

2. **日付の配車を第N週で絞っていた**
   9/1 は第1週。第2〜5週の設定を持つ人が**全員落ちていた**。
   → `soge-day-guard-v1`。日付の配車では第N週で絞らない。

3. **状態が読めないときに作り直していた**
   読めない＝無い、として扱っていたので、打刻の入った日でも作り直され、
   **消えた跡も残らなかった**。
   → 読めなかったら何もしない。消すときはログを出す。

★ 10:06 に誰かが作り直した跡（`created_at` が全件同じ）は残っているが、
　**それが何だったかは未解決**。Cloud Run のログ待ち。

### 出したもの

```
soge-tap-retry-v1     押しても何も起きないのを止めた／圏外の打刻を貯めて送り直す
soge-day-guard-v1     日付の配車を第N週で絞らない／読めないときは作り直さない
soge-day-merge-v1     走り出したあとでも配車を直せる（差分だけ当てる・打刻は消さない）
soge-plan-label-v1    先の人の時刻に「予定」と書く（打刻済みに見えていた）
soge-time-edit-v1     時刻が軽く触れただけで確定するのを止めた
soge-back-v1          運転席で順番を入れ替える（後にする）
soge-done-road-v1     送り終わった人を道の上に残す（「済 08:41」）
soge-run-list-v1      運転中／一覧の2つにした（車両から・便から を廃止）
soge-run-acc-v1       運転中の車えらびを縦のアコーディオンに
soge-run-pick-v1      はじめは全部閉じる／一覧は車を選んでから
soge-trip-pick-v1     一覧を 便→車→一覧 の順に（便は横3つ）
soge-print-driver-v1  記録表に運転手の列
soge-driver-change-v1 土壇場で運転手を替えられる
soge-undo-v1          押し間違いを戻せる
soge-drive-v5         運転中の板：先の人を灰色に
soge-week-head-v1     配車編集を「切り替え／操作」の2段に・言葉を7か所そろえた
soge-week-gap-v1      操作の段と曜日の段のすき間（実測0pxだった）
soge-week-preview-v1  配車編集に見るだけのプレビュー（便で見る／車で見る）
soge-week-pvmx-v1     「車で見る」を表にして、便の頭をそろえた
soge-week-warn-v1     「作り直されません」が嘘になっていたのを直した
mon-no-greeting-v1    モニタリングのAI文から前置き・結びを消す
a2-voice-v1 / a2-len-v1   評価担当ページに音声文字起こし／文字数150〜500
a2-section-delete-v1  ページごとの削除ボタン
a2-seq-swap-v1        評価担当ページの1と2を入れ替え（聞き取りが先）
```

### ★この日の失敗（次に読む自分へ）

**うまくいった話だけ書くと、同じ穴にまた落ちる。だから残す。**

| 失敗 | 学び |
|---|---|
| 関数を目印の**うしろ**に足して、その関数の**中**に入った。画面が真っ白（`escA is not defined`）。`node --check` は通った | **必ず前に足す**（`add_before`）。足したあと、ひとつ上が `}` `;` `*/` で終わっているか assert |
| assert が**自分の書いた説明コメント**に当たる。**10回以上やった** | 「文字列が入っているか」で数えない。**行まるごと**（`count_line`）か**コードの形**で見る。`def` の行も数に入る |
| 一覧の **⇄ と ＋利用者 が押しても何も起きなかった**。パネルの置き先が見つからず落ちていた。落ちても画面には何も出ない | 置き先には**必ずフォールバック**。そして**作ったら自分で押す**。読んで正しそう、で出さない |
| **「作り直されません」が嘘になった**。動きは変えたのに、それを説明する文を直し忘れた。HIROさんが文を読んで気づいた | **動きを変えたら、それを説明している文を全部探す** |
| 列数を**画面の幅**で決めたら、名前が1文字ずつ縦に割れた。本文は `--page-max-width` で 480px に絞られていた | **入れ物の幅**で決める（`@container` / `auto-fit + minmax`） |
| 「Supabaseの1000行制限だ」と言い切った → SQLで**0件**。外れ | **言い切る前に測る**。外したら、はっきり外れたと言う |
| スクリーンショットの灰色の板の時刻を「打刻済み」と読んだ → **予定時刻**だった | 画面の色や数字から推測しない。データで確かめる |
| 作り物データの間違いで、**確かめたつもりで何も測れていなかった**（`facility_code` 入れ忘れ／`id` の重複で別人のパネルを開いた） | fixture は本物と同じ形にする。**測った値が予想と違ったら、まず fixture を疑う** |

### この日の決めごと（HIROさん判断）

- 打刻が付いた人を配車から外しても、その人の記録は**残す**（実際に乗せたので）
- 当日の配車変更は**保存したら自動で反映**。いつでもできるようにする
- 運行画面の見せ方は**運転中と一覧の2つだけ**。事務所用の見せ方は消す
- 配車編集のプレビューは**見るだけ**。編集は今の画面で足りている
- 画面の呼び名：曜日ごと＝**いつもの配車**／日付ごと＝**今日だけの配車**

---

## モニタリングの挨拶文と、評価の音声の保存 2026-09-01（夜）  <!-- session-2026-09-01-night -->

本番 `ce00bbb`（`tasukaru`）まで反映ずみ。dev（`7c98423`）と揃っている。
`pyflakes` は **89件のまま**（基準を超えていない）。

### 出したもの

```
eval-audio-nostore-v1     評価ページの音声を Storage に保存しない（文字起こしに使うだけ）
mon-no-greeting-print-v1  書類出力の自動生成にも「挨拶を書かない」を入れた
mon-strip-read-v1         保存済みの報告書を【読むとき】に挨拶を外す（DBは変えない）
```

### ① 評価の音声が、消えないまま溜まっていた（`eval-audio-nostore-v1`）

`/api/evaluation/ingest_file` に「ストレージに一時保存（評価確定時に削除）」と
書いてあったが、**その削除はどこにも実装されていなかった**。

- 保存先は `assessment-audio/{施設コード}/{ランダムUUID}.{拡張子}`
- `upload_audio_to_supabase()` の**戻り値(URL)を捨てていた**ので、
  どのファイルが誰の・何月のものか、どこにも残らない
- 削除の口は `/api/meeting/audio_cleanup` と `/api/meeting/audio_delete` の2つだけ。
  どちらも `{施設コード}/{エリア}/{セッションID}/` の下しか見ない。
  評価の音声は**その外側**にあるので、一生消えなかった

→ **保存そのものをやめた。** 音声はその場で文字起こしに使うだけ。
　元のファイルは職員の端末に残っているので、取り直しにも困らない。

★**入れ直したくなったら、先に「どのファイルが誰のか」を残す仕組みを作ること。**
　それが無いまま保存すると、また消せなくなる。

★`upload_audio_to_supabase` を実際に呼んでいるのは、いま**掲示板の投稿だけ**。
　他の5か所は import しているだけで使っていない。

### ② 書類出力から作った月だけ、挨拶が付いていた（`mon-no-greeting-print-v1`）

8/31 の `mon-no-greeting-v1` は、**モニタリング画面側（`api_generate_monitoring`）
にしか入れていなかった**。書類出力の自動生成（`_auto_generate_monitoring`）は
**別の `BASE_PROMPT` を持っている**ので、禁止の2行も `_mon_strip_greeting()` も無かった。

しかも自動生成の結果は `monitoring_reports` に**保存される**ので、
そのあとモニタリング画面を開いても挨拶付きの文が出る。
＝**どちらの入口から先に作ったかで、ケアマネジャーに渡る文章が変わっていた。**

★教訓：**同じことをする場所が2つあるなら、直すのも2つ。**
　片方だけ直すと、入口によって結果が変わる。これは8/28の「言葉の失敗」と同じ形。

### ③ すでに保存済みの挨拶（`mon-strip-read-v1`）

①②を直しても、**8/31より前に作られた報告書の挨拶は残ったまま**。
本番で数えたら、cocokaraplus-5526 の 2026-05〜08 だけで**LIMIT 50 に当たった**
（他の85事業所ぶんは未計測）。

→ **読むときに外す**ことにした。外す場所は4か所。

```
/api/monitoring_detail        モニタリング画面の履歴（★手直しもここから読む）
/api/monitoring_report_data   モニタリング画面の報告書シート
/print_pdf                    書類のPDF
/print_preview                書類の印刷プレビュー
```

- **DBは1件も変えない。** 読んだものを渡す前に外すだけなので、いつでも戻せる
- 手直し（インライン編集）も同じ道で受け取るので、
  **「画面では消えているのに編集を開くと挨拶が戻る」食い違いは起きない**。
  手直しして保存すれば、そのときDBの側もきれいになる

★**消してはいけない文は守られている**（実測ずみ）。
　「今月このカテゴリの報告はありませんでした」（40文字未満なので触らない）と
　「【この行は書類には印刷されません…】AIの生成に失敗しました…」
　（前置き・結びのどちらの形にも当たらない）。
　**ここを緩めると `gen-fail-visible-v1` の不具合に戻る。**

### 確かめ方（この日にやったこと）

| 対象 | やったこと | 結果 |
|---|---|---|
| 外す処理そのもの | 本物の `app.py` から **AST で切り出して**、本番から出てきた実データ50件を通した | 38件で挨拶が消え、12件はそのまま。「報告なし」「生成エラー」「ご家族へご報告いたします」は無傷 |
| パッチ | 3回ずつ流した／`py_compile`／`pyflakes` | 二重に入らない・OK・**89件のまま** |
| DEMO001（読むだけ） | 報告書29件・カテゴリ欄228個 | 先頭の挨拶0・末尾の挨拶0・「報告なし」149行が無事 |
| DEMO001（書き込みテスト） | 青木 利夫様 2026-08 の「その他」に**わざと前後の挨拶を足して保存**し、開き直した | **両方消えた**。他7カテゴリは無傷。**元の文に戻した** |
| 本番（読むだけ・GETのみ） | SQLで挨拶入りと分かっていた行を**名前と月で狙い撃ち**して読んだ（井立様8月・日高様8月・奥村様7月・森川様7月・下川様6月ほか8件） | **8件すべて、DBには挨拶があるのに画面には出ていない** |

★**本番では `GET` しか叩いていない。**`/print_preview` と `/print_pdf` は
　**わざと開いていない**（報告書が無い月を開くと自動生成が走り、書き込みになるため）。

### ★この日の失敗（次に読む自分へ）

| 失敗 | 学び |
|---|---|
| `swap()` の分岐を**逆に書いた**。「新しい形が古い形を含むなら古い形を先に見る」としたので、古い形が新しい形の中に残るパッチで**2回流すと二重に入った** | 正しくは【**新しい形が古い形の一部**なら古い形を先に】。引き継ぎに書いてあった通りの失敗を、書き写すときに反転させた。**helper を写すときこそ、2回流して確かめる** |
| 「本番マージが Already up to date なのはおかしい」と**言い切った**。実際は1回目のマージが成功していて、貼られたのは2回目の実行だった | **言い切る前に測る。**`git log` を見てから言えばよかった。8/28の「1000行制限」と同じ形 |
| `count_line(text, "        from utils import get_generative_model") == 1` で assert した → その行は app.py に**15行あった** | 既存コードの行数で数えない。**自分が足した目印の行**で数える |
| DEMO001 で「挨拶が0件だから直った」と言いかけた。DEMO001には**そもそも挨拶が1件も無かった** | **消える対象が無い場所では、消えたことを確かめられない。**「無い」と「消した」は違う |

### この日の決めごと（HIROさん判断）

- 評価の音声は **Storage に保存しない**。文字起こしに使うだけ
- 挨拶文は **①これから作るぶんを直す ＋ ②保存済みのぶんも読むときに外す** の両方
- **DBの中身は書き換えない**（読むときに外すだけ。だから戻せる）

### 残っていること（2026-09-01 夜の時点）

| 件 | 状態 |
|---|---|
| `assessment-audio` にすでに溜まっている音声 | **未着手。** 新しく増えるのは止めた。既存は量も数えていない。数える→消す、の順で |
| `/api/monitoring_report_data` が `monitoring_reports` を **`year_month`** で引いている | **未確認。** 他6か所は全部 `target_month`。もし `year_month` 列が無ければ毎回 `except` に落ちて**本文が空のまま返っている**。列を見る読むだけSQLを流して確かめる（app.py 31416行） |
| 保存済みDBの挨拶を実際に消すか | 保留。いまは読むときに外しているだけ。DBを直す UPDATE は本番86事業所への書き込みなので、必要になってから |
| リポジトリ直下の 0バイトファイル `tasukaru-dev` | `git log tasukaru-dev` が「ブランチ名かファイル名か分からない」で止まる。害は無いが消すと楽 |
| 9/1 10:06 の配車の作り直しが何だったか | **未解決。** Cloud Run のログで `soge day state error` が 2026-09-01 01:06 UTC 前後に出ていないか |
| 配車編集の「確定を解除する」ボタン | いまは運行画面の「一覧」にしか無い。置き場所はHIROさん判断待ち |
| 「車で見る」で人数の多い便（13名など） | 縦に長くなる。折りたたむかどうかは実データを見てから |
| 席が足りないまま保存できる | 未着手 |
...
| px直書きが残り約80か所 | 未着手 |
| 書類出力（担当者会議用）の挨拶文 | **対応ずみ**（`mon-no-greeting-print-v1`） |
| `/api/evaluation/ingest_file` の音声が「評価確定時に削除」されているか | **確認ずみ＝されていなかった**。`eval-audio-nostore-v1` で保存自体をやめた |

---

# 2026-09-02（火）送迎まわりを頭から洗い直した日

この日は送迎（そうげい）だけをやった。**現場から上がった不具合を直す → その確認中に
もっと重いものを踏む → コードを頭から読み直したらさらに7件出てきた**、という順番。
`tasukaru-dev` に6本、本番に4本入れた。

## 入れたもの

| marker | 何を直したか | 本番 |
|---|---|---|
| `soge-touch-edit-v1` | 並べ替え・車の移動・打刻の取り消しも「動き出した」に数える（`edited_at` を見る） | 済 |
| `soge-merge-btn-v1` | 運行画面に「**配車表を取り込む**」を追加。打刻を残して並びだけ当てる | 済 |
| `soge-stable-id-v1` | 手つかずの日を「消して作り直す」から「**差分だけ当てる**」に変更 | 済 |
| `soge-addr-none-v1` | 住所が文字列 `"None"` のとき出さない／京都の住所が頭から欠けるのを直す | 済 |
| `soge-tap-refuse-v1` | 断られた打刻を圏外扱いで貯めない／列が詰まらない／送信中の打刻が消えない | 済 |
| `soge-fresh-word-v1` | 画面が前面に戻ったら読み直す／日をまたいだら日付を今日に／文言 | 済 |
| `soge-guard4-v1` | 過ぎた日を作らない／出発帰着のガード／運転手を守る／失敗を成功と言わない | 済 |

## ★この日いちばん大きかったもの — `soge-stable-id-v1`

**運行画面を読むたびに、立ち寄りの行が丸ごと作り直されて id が全部変わっていた。**

`soge_materialize_day` の docstring にこう書いてあった。

```
・それ以外 → 開くたびに作り直す＝配車表の直しがそのまま出る
```

狙いは正しいが、**1人しか見ない前提**の作りだった。実際にはこうなる。

1. 職員Aがタブレットで運行画面を開く（画面は id のセットAを持つ）
2. 職員B（別の端末・別のタブ・記録表・印刷）が同じ日を開く → **行が作り直され、セットAは消える**
3. Aが「到着」を押す → `stop_id` が見つからず弾かれる → グルグル → スタートの状態に戻る

**DEMO001 で 0.6秒あけて2回読んだだけで、3人ぶんの id が全部別物になった。**

さらに悪いのは、弾かれた打刻が端末の送り直しの列（`soge-tap-retry-v1`）に積まれること。
あの列は**死んだ `stop_id` を持ったまま**送り直し続けるので、電波が戻っても永久に入らない。
**打刻が黙って消える。**

直し方：作り直しをやめて、既にある `_soge_merge_day`（差分当て）に回す。行を消さないので
id が変わらない。ついでに「中身が同じ行は書かない」を足して、読むだけの回は読むだけで終わるようにした。
`force`（ゼロから作り直す）は今まで通り消して作り直す。

**確認**：14人の日を0.7秒あけて3回読んで、全員 id が同じ。配車表を直すと並びは変わるが、
**編集の前後で既存の行の id は同じまま**。

## ★9/1 の不具合の原因は2つあり、片方は前日の自分の直しだった

「配車編集で順番を変えても運行画面に反映されない／『直前のものは変更できない』と出る」

- **原因① 前日入れた `soge-touch-edit-v1` が判定を広げすぎた。**
  打刻が1件も無くても、並べ替えただけで「動き出した日」になり、
  配車表からの自動反映が止まっていた。**自分のミス。**
- **原因② そもそも「いつもの配車」（週次）の保存は、当日の運行表に何もしていなかった。**
  これは前からある穴。

自動で反映させる案は**採らなかった**。自動にすると、運転手が「後にする」で組み替えた並びが
画面を開くたびに配車表どおりに戻る。今回の不具合より悪い。
代わりに**押したときだけ取り込むボタン**を足した。

## この日わかった「今どう動くか」（HIROさんとの整理）

分かれ目は「その日の運行画面をもう触ったか」。

**A. まだ何も起きていない日** … 今日だけの配車も、いつもの配車も、**どちらも入る**

**B. もう打刻や並べ替えが入っている日**

| 直したもの | 運行画面に入る？ |
|---|---|
| 今日だけの配車 | **保存すれば、そのまま入る**（打刻は残ったまま） |
| いつもの配車 | **入らない** →「配車表を取り込む」を押せば入る |

理由：「今日だけの配車」は**その日を名指しして**保存するので、保存の瞬間にその日へ当てられる。
「いつもの配車」は**毎週の型**なので、保存しても「どの日に当てるか」を持っていない。

★HIROさんの判断：**自動で当てにいかない。**「毎週の型を直しただけなのに今日が変わった」は
送迎の最中だと事故のもと。ボタンを押した人が決める。

## 画面の文言（HIROさんの文）

**運行画面のバナー（手が入った日）**
> この日はもう、運行画面で**打刻や並べ替え**がされています。
> **いつもの配車**を変更しても、運行画面には反映されません。運行画面を変えたい場合は**「今日だけの配車」**から変更してください。
> すでに直した配車表を、この日に取り込みたい場合は、下のどちらかを押してください。

**今日だけの配車の緑の帯**
> この日はもう、運行画面で**打刻や並べ替え**がされています。保存すると、当日の運行表にも反映します（すでに入っている打刻はそのままです）。

★元は「この日はもう**打刻が始まっています**」だった。打刻ゼロで並べ替えただけの日にも出るので嘘になる。
★「手が入っている」に書き換えようとしたが、**HIROさんに「伝わりにくい」と言われた**。
　実際に多い2つ（打刻・並べ替え）を名指しするのが正解だった。

## 打刻を貯める列にあった3つの穴（`soge-tap-refuse-v1`）

`srSend` は返事を2種類に分けて返していた。

```
{ ok:false, off:true  } … 本当に届いていない（圏外）→ あとで送れる
{ ok:false, off:false } … サーバが断った（403/404/ログイン切れ）→ 何度送っても入らない
```

**ところが呼ぶ側がどこも `off` を見ていなかった**（`grep` してもゼロ）。

1. 断られた打刻でも画面に時刻が入り「電波が戻ると自動で送ります」と出る。**永久に入らない。**
   しかも `left = a.slice(i); break;` なので、**断られる1件が先頭に居座るとそのあとの打刻が全部止まる。**
   → 前の晩から開きっぱなしのタブレットで朝いちに押すと、昨日の日付あてで403、その日の打刻が丸ごと止まる。
2. `if (!hs) { continue; }` — 消えた立ち寄りあての打刻を**黙って捨てていた**。
   まだ送れていない打刻は DB では `arrived_at` が空なので、`_soge_merge_day` の削除条件をそのまま満たす。
3. `srqSave(left)` で列を**丸ごと書き戻していた**。途中に `await` が何度も入るのに `srqAdd` は
   止まっていないので、**送信中に押した打刻が消えた。**

## 開きっぱなしの画面が永久に古いままだった（`soge-fresh-word-v1`）

**HIROさんの指摘から見つかった。**「リロードするまでタイムラグがある」と言われて確かめたら、
自動で読み直す仕掛けが**一つも無かった**。あるのは

```js
setInterval(function () { if (srqLoad().length) srqFlush(false); }, 30000);
```

だけで、これも「送れていない打刻が溜まっているとき」しか動かない。
**タイムラグではなく、開きっぱなしの画面は誰かが読み込み直すまで永久に変わらない。**

日付も同じ。`state.date` は起動時に1回入れるだけなので、前の晩から開きっぱなしだと
朝も「昨日」のまま。そのまま押すと403。

直し：`visibilitychange` と `focus` で読み直す。**読み直さない場合を先に決めた**——
送信中／時刻を手で入れている最中／ボタンを押している最中／前に読んでから8秒以内。
**見ていた位置は保つ**（運転席で先頭に飛ばされるのがいちばん困る）。

## 残り4つ（`soge-guard4-v1`）

- **過ぎた日でも「運行表がまだ無い日」は、いまの配車表から作られていた。**
  歯止めが `if st["exists"]:` の**中**にあったので、運行表の無い過去日は分岐に入らず落ちていた。
  日付欄に先月の日を入れて開くだけで、**走っていない日が月間の運行記録表に載る。**
- **`/run/depart` と `/run/return` だけ `_soge_past_admin_ok` を通していなかった。**
  他5本（arrive / stop / day / move / extra）は全部通していた。
- **運転手を替えても、画面を開き直すと配車表の運転手に戻っていた。**
  ★`_soge_day_touched` に `driver_name` を足すのは**間違い**。配車表から作った日は最初から
  `driver_name` が入っているので、作った瞬間に touched になり配車表の直しが一切届かなくなる。
  正しくは「その日にもう運転手が入っているなら、配車表で上書きしない」。
- **書き込みの例外を握りつぶして、最後は必ず `built: True` を返していた。**
  `force` は先に全部消してから作り直すので、作る側が落ちると**打刻も臨時便も消えたうえに空になり、
  「配車表から作り直しました」と出る。**失敗を数えて `built: False` を返すようにした。

## 住所の `None`（`soge-addr-none-v1`）

運行画面の次以降の行に文字列 `None` が出ていた。**コードではなくデータ**で、
`patient_profiles.address` に Python の `str(None)` が4文字で入っていた。

**本番には1件も無い**（読むだけSQLで確認、0行）。DEMO001 のデモデータだけ。
`app.py` の中に書き込んでいる場所も見つからなかった（過去の取り込みスクリプトの跡）。

見え方の応急処置として、`_soge_addr_short` で `None/null/nan/undefined/-` を空として扱うようにした。
ついでに **京都府・京都市の住所が頭から欠ける**（「先頭5文字に都/道/府/県が出たら切る」なので
京**都**府の2文字目で切れる）のも直した。

## 確かめ方（この日にやったこと）

| 対象 | やったこと | 結果 |
|---|---|---|
| 立ち寄りの id | DEMO001 で 0.7秒あけて3回読む | 14人ぶん**全部同じ id**（直す前は全部変わっていた） |
| 配車表の反映 | 週次で阿部 武を先頭に動かして保存 | 並びは変わり、**既存の行の id は前後で同じ** |
| 過ぎた日 | 同じ木曜の過去日を4つ開く | 8/27 は14人（実際の記録・無事）、8/20・7/30・6/25 は**0人＝作られない** |
| 運転手 | 便に名前を入れて2回読み直す | **残った**（`touched` が false のままでも守られる） |
| 打刻の列 | app から抜き出して **Node で19通り** | 圏外は残る／拒否は列から外れて後続が送られる／消えた立ち寄りは名前が出る／送信中に押した分が残る … 全部OK |
| 前面に戻る | 抜き出して **Node で16通り** ＋ 実機で `focus` を投げる | 読み直す／8秒以内の連打は無視／隠れていたら動かない／位置を保つ … 全部OK |
| 住所 | 関数を抜き出して **27通り** | None系10通り＋実在の住所17通り（神奈川県・和歌山県・鹿児島県・県道を含む）全部合格 |
| 運転手のルール | **7通り** | 新しい便／未設定／None／空白／土壇場の交代／配車表が空／同じ名前 … 全部合格 |
| 過ぎた日の歯止め | **AST で構造を検査** | `past` が関数本体の直下にあり `if st["exists"]` より前、その中に `past` は無く `locked`/`touched` は残る |
| 全パッチ共通 | 3回ずつ流す／`py_compile`／`pyflakes`／Jinja2 パース／`node --check` | 二重に入らない・OK・**89件のまま**・OK・OK |

## ★確かめられなかったこと

**出発・帰着の「過ぎた日は管理者だけ」が403を返すところ。**
DEMO001 のデモ職員Aが**管理者**だった（`can_edit: true`）。管理者は通す仕様なので、
このアカウントでは403が出ない。**管理者でない職員のログインが要る。**
無理に過ぎた日へ書き込むのは記録を汚すのでやめた。
→ 現場で一般職員のアカウントから、昨日の便で「帰着」を押して
　「過ぎた日の記録は、管理者だけが直せます」と出れば正解。

## ★この日の失敗（次に読む自分へ）

| 失敗 | 学び |
|---|---|
| **自分の確認作業が、確認しようとしていたデータを壊していた。** `/api/soge/run` を叩いて状態を見たら、その GET 自体が当日の行を作り直していて、開いていた画面の id が死んだ。最初の再現テストを自分で潰した | **読むつもりの操作が書いているかどうかを、先に確かめる。** ただし結果的にこれが `soge-stable-id-v1` の発見につながった。**壊れ方を観察するのは無駄ではない** |
| `count_line` の assert で**インデントを3回間違えた**（`keep_absent` は16字下げなのに12字、`past` の戻りは8字下げなのに4字…） | 行を数えるなら、**先に `grep -n` で実物を見てから書く。**頭の中のインデントは当てにならない |
| **自分の書いたUIの説明文が2回とも伝わらなかった。**「取り込みかたは2つあります」→「どういうこと？」、「手が入っている」→「伝わりにくい」 | **書いたら声に出して読む。**「2つ」が何を指すか、書いた本人以外に分かるか。**現場の言葉は現場の人が正しい**（HIROさんの「打刻や並べ替え」が正解だった） |
| `doMerge` に `confirm()` を使い、**ブラウザ自動化が固まった** | 既存の `doRebuild` に合わせたので作りとしては正しいが、**確認の段取りを間違えた。**自動化で確かめるなら `window.confirm` を差し替えてから押す |
| 最初の3件を直して**満足しかけた。** HIROさんに「全部潰した？」と聞かれて読み直させたら、**さらに7件出た** | **「直したもの」と「残っているもの」は別。**踏んだ不具合を直しただけでは「潰した」ではない。**同じ場所を頭から読み直す時間を必ず取る** |
| 前日入れた `soge-touch-edit-v1` が、**翌日の不具合の原因の半分**になっていた | 判定を広げる直しは、**広げた側で何が止まるかを必ず数える。**「動き出した」に数える＝「自動反映が止まる」でもある |

## この日の決めごと（HIROさん判断）

- 「いつもの配車」の保存を**自動で当日に当てにいかない**。押した人が決める
- 画面の文言は**「打刻や並べ替え」**と実際に多い2つを名指しする（「手が入っている」は使わない）
- 住所の `None` は**データを消さない**（本番に無いため。見え方だけ直す）
- 本番の運行画面は**開かない**（開くだけで当日データに書き込みが起きうるため）。現場で確認してもらう

## 残っていること（2026-09-02 の時点）

| 件 | 状態 |
|---|---|
| **曜日配車の保存が「全消し→入れ直し」** | **未着手。** `/api/soge/week` PUT（26608行）と `/api/soge/date` PUT（26932行）。insert が落ちると delete だけ効いて**その曜日の配車が消える**。手つかずの日は運行画面を開いた瞬間に自動生成で組み直されるので、別物になる。踏むには insert 側の失敗が要るので優先度は低め |
| 出発・帰着のガードの403 | **現場で確認待ち**（一般職員のアカウントが要る） |
| `_soge_merge_day` の `stop_by_key` が `(patient_id, stop_type)` だけ | 同じ人が同じ種別で2つの便に入っていると鍵がぶつかる（27808行）。DBの制約が弾いて例外が握りつぶされ、実害は「その行だけ並びが追従しない」程度 |
| `/api/soge/run/replan` の `soge_stops` 更新に `facility_code` の絞りが無い | 28660行。id が UUID なので実害の筋は立たなかったが、揃えておきたい |
| 9/1 10:06 の作り直しの正体 | **ほぼ解けた。**`soge-stable-id-v1` で見つけた「誰かが同じ日を開いただけ」で説明がつく。裏を取るなら Cloud Run のログで `[soge-guard] ... を作り直します（便N 打刻M）` を見る。打刻Mが0でなければ、その時に打刻が消えている |
| `assessment-audio` にすでに溜まっている音声 | **未着手**（新規は止めた、既存は未計測） |
| `/api/monitoring_report_data` が `year_month` で引いている | **未確認。** 他6か所は `target_month`（app.py 31416行） |
| リポジトリ直下の 0バイトファイル `tasukaru-dev` | `git log tasukaru-dev` が止まる。害は無いが消すと楽 |
| 席が足りないまま保存できる／端末を全部消すと承認ログインが使えない／LINE送信の成功ログ／`my_tasks_overdue` の JSON エラー／旧ログインを閉じる／px直書き約80か所 | 未着手 |

## 営業資料（2026-09-05 金 プレゼン用）

`~/dev/TASUKARU_営業資料/` に置いてある。

- `TASUKARU_ご提案資料.pptx` … **HIROさんが自分で添削したもの。これが正**
- `TASUKARU_ご提案資料.pdf` … 上から書き出したもの
- `TASUKARU_ご案内_A4.pdf` … A4両面
- ★古い `TASUKARU_ご提案.pptx` / `.pdf` は**消していい**

★**「86事業所が毎日使っています」は嘘なので書かない。**引き継ぎに書いてあったが裏が取れていない。
★pptx を**プログラムから書き換えない**。python-pptx が `[Content_Types].xml` から
　`<Default Extension="jpg">` を落として壊したことがある。HIROさんの手元の pptx が正。
★ROI の試算は**AIによる試算**と明記してある。ココカラプラスの実測が出たら差し替える。

---

# 2026-09-04（金）目標期間・実績集計・勤怠を通しで直した日  <!-- session-2026-09-04 -->

**本番まで入った**（`5cfefa8` / `03a5ba6` / `caa3169` / `e196c2d` / `ba45ad3` / `9c6453f` / `073e2a8`）。

## 入れたもの

| marker | 中身 | 触った所 |
|---|---|---|
| `sw-no-report-cache-v1` | **一括印刷で古いデザインが出る**のを止めた | `static/sw.js` |
| `soge-tap-input-v1` | 送迎の時刻の手入力・候補一覧のタップ | `soge_run.html` |
| `soge-print-leave-v1` | 記録表に「休」を出す（板倉さんの件） | `app.py` / `soge_print.html` |
| `soge-lock-help-v1` / `soge-lock-word-v1` | 確定ボタンの［？］と案内文を揃えた | `soge_run.html` |
| `soge-guide-flow-v1` | 送迎の「？」に使う順番を載せた | `guide_integration.py` |
| `mon-fitness-1year-v1` | 体力測定・体重の取得を「1年以内・直近3件」に | `app.py` |
| `report-values-v3` | 体力測定・体重の見せ方を縦1行ずつに | `print_preview.html` |
| `soge-plan-manual-v1` / `-ui-v1` | 到着予定時刻の手入力（翌週も引き継ぐ） | `app.py` / `soge_week.html` |
| `soge-list-type-v1` / `soge-run-only-v1` | 一覧の迎え／送りの色分け・この車両のみ表示 | `soge_run.html` |
| `soge-print-plan-v1` | **まだ運行表が無い日**を予定として記録表に出す | `app.py` / `soge_print.html` |
| `soge-print-word-v1` | ボタン「欠席にする」→「休みにする」（説明文も） | `soge_print.html` |
| `goal-period-endmonth-v1` | 目標期間の終了日を「開始月から N か月目の**月末**」に | `assessment.html` |
| `goal-period-carry-v1` | **期間だけ変えたときの「期間を繰越で変更しますか？」** | `assessment.html` |
| `goal-period-endrule-v1` | 終わり方と月数を**管理者MENU**で設定できるように | `app.py` / `admin.html` / `patient_profile.html` / `assessment.html` |
| `jisseki-demog-v1` | 実績集計表に**男女比・曜日別（午前/午後/終日）** | `app.py` / `admin_jisseki.html` |
| `jisseki-demog-charts-v1` | 曜日別のグラフ3つ（予定／実績と予定／曜日別男女比） | `admin_jisseki.html` |
| `jisseki-demog-jin-v1` | 曜日別に**実人数**を足した（延べと両方） | `app.py` / `admin_jisseki.html` |
| `jisseki-unknown-who-v1` | 「午前・午後が分からない来所」に**誰のどの日か** | `app.py` / `admin_jisseki.html` |
| `jisseki-print-demog-v1` | 印刷ページにも同じ表とグラフ | `admin_jisseki_print.html` |
| `pay-leave-rows-v1` | 給与出力5本に**打刻のない休みの日**を出す | `app.py` |
| `timecard-allday-v1` | 勤怠集計の画面に**月ぜんぶの日**を並べる | `admin_timecard_report.html` |
| `pay-allday-rows-v1` | 出力5本にも**何も無い日**を並べる | `app.py` |

## ★この日いちばん大きかったもの — Service Worker が印刷のHTMLを溜めていた

**症状。** テンプレートに「cocokaraplus」を選んで**全員まとめて**印刷すると、
**古いデザイン**（直近6ヶ月・青いベタ塗り見出し）が出る。
同じタイミングでも**1人ずつ**印刷すると新しいデザインが出る。

**原因。** `static/sw.js` の `isHtmlRoute()` は
「同一オリジンで拡張子が無い」なら Network-First にするが、
**成功時に `caches.put` している**。`caches.put` は `Cache-Control` を見ない。
`after_request`（`app.py` 128行）が `no-store` を付けていても**素通り**する。

**なぜ一括だけか。** `print_output.html`（712行）は
**全員を選ぶと `user_names` を付けない**ので、一括のURLは毎回**同じ文字列**になる。
だから確実に当たる。1人ずつは名前が入るので当たらない。

**確かめ方。** 本番のブラウザで（読むだけ）キャッシュを列挙したら、
v32 に `/print_preview` が**23件**入っていて、うち1件に
「直近6ヶ月」が**3回**出てきた＝直す前のHTMLがそのまま残っていた。

**直し。** `NO_CACHE_PATHS = ['/print_preview','/print_pdf','/print_output']` を作り、
成功時に `caches.put` しない・失敗時はオフラインページを返す。`CACHE_VERSION` を v33 へ。

★**教訓。** サーバで `no-store` を付けても、Service Worker が自分で溜めるのは止まらない。
　**「no-store を付けたから大丈夫」は Service Worker がある限り成り立たない。**

## ★終了日の計算が2つの画面にあって、片方だけ直しかけた

目標期間の終了日を計算するコードは

- `templates/assessment.html`（評価ページ）
- `templates/patient_profile.html`（利用者基本情報・**初回入力**）

の**2か所**にあった。評価ページだけ直していたら、
**初回に入れた終了日と、評価で入れ直した終了日が食い違う**ところだった。

`goal-period-endrule-v1` で `_goalPeriodEnd()` として同じ形にそろえ、
パッチの検査で**2つの画面のコードが1文字までそろっているか**を確かめている。

★「開始日の前日まで」を選んだときの結果が従来と一致するかは、
　**2,400通り**（4年×12月×日×月数1/3/6/12/24）で突き合わせて**差ゼロ**。

## ★実人数は足し算では出せない（本番の実データで出た）

実績集計表の曜日別。本番2026年8月の**実人数の合計は70人**だが、
各列（午前のみ・午後のみ・終日）を足すと**78**になる。
差の8人は**同じ月に午前と午後の両方に来ている方**。

なのでサーバ側で**人の集まり（set）で持ってから数え**、
曜日の合計も月の合計もそこから出している（`jisseki-demog-jin-v1`）。
画面側も、実人数の合計は各列を足さずにサーバの数字を使う。

★**「合計＝各列の和」だと思った瞬間に間違える。**
　同じ人が2つの枠に入りうる集計は、必ず集まりで数える。

## ★私が指示を読み違えていた（給与の件）

9月3日の「打刻のない休みの日が表示されない」を**給与出力の話**だと読み、
出力5本を直した（`pay-leave-rows-v1`）。あれはあれで本当に穴だったが、
HIROさんが見ていたのは **`/admin/timecard/report` の画面**で、
しかも足りないのは「休み」ではなく**何も無い日の行そのもの**だった。
加藤鮎美さんの2026年8月で **8/1(土) が丸ごと飛んでいた**。

★`s.days` に入るのは「**打刻のある日**」＋「**staff_leave_days に登録のある日**」だけ。
　この画面や出力で**日が飛ぶ**ときは、まずここを疑う。

直したもの

- `timecard-allday-v1`（画面）… その月を1日から末日まで並べる
- `pay-allday-rows-v1`（出力）… 締日ぶんの期間を1日も飛ばさずに並べる

★どちらも**「公休」とは書かない**。公休なのか登録し忘れなのかは
　データからは決められないので、決めつけずに空欄で出す。

## ★お金の計算に触らずに行を足した（給与出力）
...
と None の日をすべての集計から外すので、実働・時間外・深夜・法定休日・
勤務日数は1分も変わらない。週40時間の判定でも「その週に働いた日」に入らない。

★**理屈だけで済ませず、実際に走らせて確かめた。**
　締め方2通り（`weekly_rest` / `sunday`）× 期間3通りで、
　休みの行を最大23件突っ込んだ状態と突っ込む前を比較 → **すべて差ゼロ**。
　1日ごとの残業・深夜・法定休日フラグも全日一致。

★パッチ自体が `_pay_compute_staff` の中身が**1文字も変わっていないこと**を
　確かめてから書き込む。お金に触れていないことを機械で保証する。

## この日の決めごと（HIROさん判断）

- 目標期間の終了日は「**開始月から数えて N か月目の月末**」（9/15開始 → 11/30）。1日開始は従来と同じ
- ただし事業所によるので、**終わり方と月数は管理者MENUに一本化**。利用者基本情報の⚙️は案内に変える
- 曜日別は**実績と予定を並べる**／**終日は別の列**／男女比は**実人数と延べの両方**
- 記録表の「まだ運行表が無い日」は**時刻は空白・名前だけ**。**どこにも保存しない**（配車表を直せば開くたびに組み直す）
- 確定ボタンは「1日の送迎が全て終わってから押す」。走っている間は押さなくてよい
- 出力にも**何も無い日**を行として出す
...
| 失敗 | 学び |
|---|---|
| **指示を読み違えて、違う場所を直した**（給与出力 vs 勤怠集計の画面） | **「どの画面か」を先に確かめる。**9月3日の時点で「特定できていない」と自分で書いていたのに、確かめずに直しに入った |
| **自己検査の assert を自分で壊した**（説明コメントに同じ字を書いた／`def` 行が数に入る／メソッド名の一部が別の所にも出る）。今日だけで5回 | **数は思い込みで書かない。当ててから数えて書く。**とくに「◯◯が残っていないこと」の検査は、自分のコメントが引っかかる |
| **本番の管理者MENUを自分で開いてしまい、LINEに認証コードを飛ばした。**そのあとDEVのセッションが `/admin` に張り付いた | **認証のかかる画面は自分で開かない。**HIROさんに通してもらってから入る |
| 印刷ページが**別のページ**だと気づかず、画面だけ直して終わりかけた | **「印刷」は別テンプレートのことがある。**画面に足したら、印刷にも同じものがあるか必ず見る |

## 仕事の進め方を変えた（HIROさん合意）

- パッチは**私が直接** `~/dev/kaigo-ai-app/` に置く。HIROさんは `python3 patch_xxx.py` を流すだけ
- コードを見たいときは**私が自分で読む**。grep や sed をお願いする往復はしない
- 中身は私が読んで書き、**流すのはHIROさん**、git も**HIROさんの手元**

★これで、日本語の書き写しミスによるパッチ不発（前日3回踏んだ）が消えた。

## 現場で直せるもの（本番の実データから分かったこと）

- **脇島由富さん** … 2026年8月の 8/7・8/21・8/28（すべて**金曜**）が
  「午前・午後が分からない来所」になっている。
  利用者基本情報の**利用曜日に金曜の登録が無い**。設定すれば警告も消える
- 本番の性別は**86人全員入っている**（女性70・男性16）。男女比はそのまま使える

## 残っていること（2026-09-04 の時点）

| 件 | 状態 |
|---|---|
| **2本線カードの「右／左」の見分け** | **紙に出して確認待ち。**見分けがつかなければ色分けか字を足す |
| 介護保険課への事前相談3件 | **未着手。**⑪を1通のままでよいか／⑬の定率・定額／生活支援廃止で老人福祉法様式第23号を出してよいか |
| ④付表第2号(3) の残り空欄 | **未着手。**管理者情報・面積・電話等 |
| 出発・帰着のガードの403 | **現場で確認待ち**（一般職員のアカウントが要る。2026-09-02 から継続） |
| 曜日配車の保存が「全消し→入れ直し」 | **未着手**（2026-09-02 から継続） |
| `_soge_merge_day` の `stop_by_key` の鍵 | 未着手（2026-09-02 から継続） |
| `/api/soge/run/replan` に `facility_code` の絞りが無い | 未着手（2026-09-02 から継続） |
| `assessment-audio` の既存の音声 | 未着手（新規は止めた、既存は未計測） |
| 席が足りないまま保存できる／端末を全部消すと承認ログインが使えない／px直書き約80か所 ほか | 未着手 |

★**確かめてもらう必要があるもの**（本番はデプロイ済み）

- 勤怠集計で加藤鮎美さんの8月に **1日(土)** が薄い行で出ているか。合計が 159時間・17日勤務のままか
- 出力（CSV/Excel/PDF）を1本落として、**画面と同じ日が同じ順**に並んでいるか
- 管理者MENU ＞ 設定 ＞ 目標期間の設定 で「開始日の前日まで」に切り替えて保存 →
  評価ページで 9/15 を入れると **12/14** になるか（保存の書き込みは未検証）

---

# 2026-09-07（月）連絡帳を5本直した日 ＋ Googleまわりの持ち主を確かめた日  <!-- session-2026-09-07 -->

## ★Googleまわりの持ち主（画面で1つずつ確かめた・2026-09-07）  <!-- readme-20260907 -->

**ここが分からないまま解約や引き継ぎをすると、本番が触れなくなります。**
「たぶんこれ」で進めないこと。以下は推測ではなく、当日この目で見た内容です。

| 何 | 持ち主 | どこで見たか |
|---|---|---|
| Google Cloud プロジェクト `TASUKARU-production`（番号 191764727533） | **`cocokaraplus@gmail.com`（オーナー）** | IAM。他は自動生成の `191764727533-compute@developer.gserviceaccount.com` だけ |
| 請求先アカウント「My Billing Account」（`019853-...`） | **`cocokaraplus@gmail.com`（請求先アカウント管理者・1人だけ）** | お支払い ＞ アカウント管理。**組織に属していない**（組織欄が「—」） |
| Google Play 開発者アカウント（$25 支払い済み） | 同上のはず（未確認） | ★次に触るとき確かめて、ここに追記すること |
| Gemini API キー | 同上（`Gemini Project` が同じ請求先にぶら下がっている） | 請求先アカウントのリンク済みプロジェクト一覧 |

- 請求先アカウントにぶら下がっているプロジェクトは4つ:
  `TASUKARU-production` / `Gemini Project` / `My Project 38406` / `ZIMAX Project`
- ★**`hiro@lifeplusllc.com` は Zoho Mail であって、Google のアカウントではない。**
  Googleまわりを探すときにここを見に行っても何も無い。

### Google Workspace は解約した（2026-09-07）

- 契約: **Business Standard / Flexible plan（月ごと）/ ライセンス1 / ユーザーあたり月額 ¥1,900**
  （開始 2026/03/06、次の請求日は 2026/10/01 だった）
- ★**TASUKARU は Workspace の機能を1つも使っていない。**
  使っているのは Google Cloud（Cloud Run / Secret Manager / Text-to-Speech）と
  Maps/Routes、Gemini API。どれも**無料のGoogleアカウントで足りる**。
  メールは SendGrid（アプリ）と Zoho（会社）で、Gmail は使っていない。
- ★**管理コンソールにログインしていたのも `cocokaraplus@gmail.com` 本人だった。**
  つまり Workspace は「このGmailアカウントに付いていた契約」で、
  解約してもアカウントそのものは消えない。
  （Chromeに他のGmailも登録されていたが、すべて「ログアウト済み」で無関係）
- 中身: **2.4GB / 2TB**（ドライブ 0.09GB / Gmail 2.31GB / フォト 0GB）。
  解約後の無料枠 15GB に収まるので、書き出し（Takeout）は不要と判断した。
- 支払いの口も別だった。Workspace は `6482-8413-...`、Google Cloud は `019853-...`。
  **Workspace を解約しても Cloud Run の支払いには一切かからない。**

### 次に同じことで迷わないための決めごと

- **`cocokaraplus@gmail.com` が本番の唯一の持ち主。**
  ここが入れなくなると、誰も本番に触れない。無料のGmailなので助けてくれる管理者もいない。
  → **2段階認証と、再設定用の電話・予備メールを維持すること。**
- 何かを解約するときは、**IAMのオーナーと請求先の管理者を先に見る**。
  「使っていないから解約」ではなく「**持ち主でないことを確かめてから解約**」の順。

## ★利用者の番号の型がばらばら（DEVのダミーで踏んだ）

```
patient_profiles.id       uuid
vitals.patient_id         text
renraku_notes.patient_id  text
line_friends.patient_id   uuid   ← ここだけ uuid
```

- 連絡帳の中は `::text` でそろうので動くが、**LINEの送信先を引くところだけ食い違う**。
- 古いデータには `"37"` のような **uuid でない番号** が残っている
  （DEVの `vitals` から拾ったらこれが来た）。その人で
  `line_friends.patient_id`（uuid列）に当てにいくと
  `invalid input syntax for type uuid: "37"` で落ちる。
- 2026-09-07 時点では**データは書き換えていない**。
  `renraku-line-items-v2` で「送信先が読めなくても文章は出す」ようにして、
  画面に「送信先を読めませんでした。このまま送らないでください」と出すだけにしてある。
- ★直すなら、古い番号を uuid に寄せる移行が要る。**別で判断すること。**

## 連絡帳を5本（すべて本番反映済み）

| marker | 何を直したか | commit |
|---|---|---|
| `renraku-line-items-v1` | 【バグ】LINEの文章に**レク・活動 / 機能訓練の中身（種目・分・メモ）/ 送迎の時刻**が入っていなかった。画面にも印刷にも出ていたのに、文章づくりだけが読み落としていた。あわせて「**この内容の文章をコピー**」を追加（LINE未導入の事業所向け。ここからは送信しない） | `29fcf59` |
| `renraku-line-items-v2` | 送信先が読めなくても**文章だけは出す**。コピーは送信しないので送信先が要らない。「送信先がいない」と「送信先が読めなかった」は文言を分けた | `eafa368` |
| `renraku-line-toggle-v1` | LINEの文章も**表示項目のトグル**を見るようにした（印刷は前から見ていた）。決まりは印刷と同じ＝個別 → 施設既定 → 全表示。**false のときだけ出さない**（未設定は出す） | `331efb6` |
| `renraku-shared-v1` / `renraku-shared-ui-v1` | **全利用者様の共通入力欄**。一覧に入り口、その日の分を1回入れれば来所者全員に効く。項目ごとに「共通を使う／この方だけ」を選べる | `c198aa1` |
| `renraku-photo-btn-v1` | 「写真を追加」が**青地に青い字**で読めなかった（うすい青の class に濃い青の背景を上書きしていた）。写真が無いときは大きな枠、入ったら小さなボタン＋枚数に | `0ffe030` |

本番マージ: `2c05500`（上4本）/ 写真ボタンは DEV まで。

### 共通入力欄の決まり（`renraku-shared-v1`）

- 置き場: **新しい表 `renraku_shared_notes`**（施設コード + 日付で1件。DEV/本番とも作成済み）
  - 共通の中身は「その日・その施設で1件」なので、利用者ごとの `renraku_notes` には置けない
- どちらを使うかの印は `renraku_notes.items.use_shared`（**列の追加は不要**）
- ★**利用者ごとの連絡帳には1行も書き込まない。** だから上書き事故が起きない
- 既定の決まり（サーバーと画面でまったく同じ）:
  1. 画面で切り替えてあれば、そのとおり
  2. 決めていなくて、その方の欄が**空**なら → 共通
  3. 決めていなくて、**書いてあれば** → その方の分
  → 3 があるので、いままで入れた分が共通に置き換わらない
- 切替は**共通に中身があるときだけ**出す。共通に切り替えても、**その方が書いた中身は消さない**（隠すだけ）
- 機能訓練だけ: 共通を使うときは**共通の文だけ**にする（その方の種目・分・メモは出さない。混ざると読めない）

## ★この日の失敗（次に読む自分へ）

- **assert の数え間違いを4回やった。** 全部「当てる前に頭で数えた」のが原因。
  - `def _renraku_visible_for(supabase, f_code, patient_id):` の**定義の行**にも当たって2
  - `data-segfor=` は 作る所・特記事項・**探す所(querySelector)** の3か所
  - HTMLの `id="x"` と JSの `getElementById('x')` は**書き方が違う**ので別々に数える
  - `navigator.clipboard` は「使えるかの判定2つ＋呼ぶ所」で3
  → **当ててから数えて書く。** 例外なし。
- **「次の @app.route まで」で関数を切り出す検査は壊れる。** 後ろに口を1つ足しただけで
  切れ目が動き、変えていないのに「変えた」と言ってしまう。
  → 字下げの無い行が来たら関数の終わり、で切る（`_top_block`）。
- **DEVのダミーデータを3回作り直した。** union で日付が文字に固まる／
  `vitals` から拾った番号が uuid でない／`::text` の付け忘れ。
  → **型を推測して書かない。** 分からなければ `information_schema` を1回読む。
- **URLを推測して渡して404を出した**（`admin.google.com/ac/users`）。
  → 画面の場所を案内するときは、**メニューの道順**を先に書く。URLは補助。

## 動画（連絡帳と掲示板）2026-09-07  <!-- readme-video-v1 -->

連絡帳と掲示板に動画を足した。本番まで出した（`fc16892`）。

| marker | 何 | ファイル |
|---|---|---|
| `video-srv-v1` | 上げる口と、知らせに送る形 | `app.py` `utils.py` |
| `video-renraku-ui-v1` | 連絡帳の画面 | `templates/renraku.html` |
| `board-video-v1` | 掲示板 | `app.py` `templates/board.html` |

列: `board_posts.video_urls` jsonb / 既定 `'[]'` / NOT NULL。**DEV・本番とも適用済み。**
連絡帳は `renraku_notes.items` の中に `video_urls` として入るので、列は足していない。

置き場は写真と同じ `case-photos`（DEV・本番とも PUBLIC・50MB・MIME Any）。
`<施設コード>/video/<uuid>.<ext>` に置く。

### ★20MB という上限はどこから来たか

**Cloud Run は HTTP/1 だと 1リクエスト 32MiB まで**（HTTP/2 なら無制限）。
これが動かせない天井。ここから引き算した。

- 写真・PDF・動画を**1つの荷物で運ぶと合算で 32MiB に当たる**
- 余裕を見て動画は 20MB（`utils.VIDEO_MAX_BYTES`／画面側 `RK_VIDEO_MAX`・`BOARD_VIDEO_MAX`）
- **★サーバーと画面の2か所に同じ数字がある。片方だけ変えないこと。**

### ★なぜ「選んだ時点で上げる」のか

掲示板は **動画を選んだその場で `/api/upload_video` に上げ、投稿には【URLだけ】を渡す。**
投稿ボタンで上げると写真・PDFと1つの荷物になって 32MiB に当たるため。
副産物として、投稿ボタンを押したあとの待ちも短くなった。

`api_board_create_post` では `request.files.get("video")` を**使っていない**。
パッチの assert がこれを見張っている。ここを崩すと 32MiB の問題が戻る。

**★動画だけの投稿は空あつかいにしない。** 空投稿ガード（`board-empty-post-guard-v1`）に
`and (not video_urls)` を足してある。画面側にも `uploadedVideos.length === 0` がある。
**両方直さないと、片方だけで止まる。**

### mp4 と mov

知らせ（LINE）の動画メッセージは `originalContentUrl` と `previewImageUrl` の**2つが必須**。
どちらも HTTPS。

- **mp4 / m4v ＋ プレビューあり** → 送る
- **mov** → 保存も表示もするが**送らない**。画面に `LINE×` の札、送信後に「◯件は送れませんでした」
- 決まりは**サーバーと画面の両方**にある（`_line_video_messages` と `rkVideoOkForLine`）。同じ形にしてある

**★2026-09-07 にHIROさんが実機で確かめた結果 ＝ mp4 だった。**
現場のiPhoneから標準の写真アプリ経由で選ぶ限り mp4 になる。
→ **ffmpeg での変換は入れないと決めた。** Cloud Run にffmpegを積むとイメージが太り、
デプロイも起動も遅くなる。ブラウザ側の ffmpeg.wasm は25MBあって古い端末で落ちる。
`.mov` が出てきたら、まず**iPhoneの 設定→カメラ→フォーマット→「互換性優先」**を案内する
（端末ごとに1回だけ・費用ゼロ）。台数が多くて回らなくなったら、そこで初めて変換を考える。

アップロードの返りには**実際の `ext`** を入れてある。現場が何を作っているのかが1回で分かる。

### プレビュー（1コマ目）

ブラウザで `<video>` → `<canvas>` に1コマ描いて jpeg にする。
**★作れなくても止めない。** `null` を返して動画だけ上げる（8秒でタイムアウト）。
プレビューが無い動画は知らせに送れないので、その旨を札で出す。
サーバー側も、プレビューの保存だけ失敗したときは動画を残す。

### ★掲示板に「LINE」と書いてはいけない

**掲示板は知らせに送らない。** `api_board_create_post` に LINE の処理は無い。
最初 `LINE×` の札をそのまま持ってきてしまい、嘘の表示になっていた。
掲示板で本当に困るのは「**他の職員の端末で映らない**」こと（mov は Android・Windows で映らない）。
→ 札は `再生できない端末あり`、下の文に「※iPhone以外の端末では映らないことがあります」。
判定も `is_mp4`（知らせ用）ではなく `d.ext` で見ている。

### 確かめた台

| 台 | 件数 | 何を見るか |
|---|---|---|
| `h_video.py` | 45 | 上げる口・知らせの形（偽Supabase） |
| `h_videoui.js` | 57 | 連絡帳の画面（偽DOM・偽fetch） |
| `h_board.py` | 33 | 掲示板のサーバー側 |
| `h_boardui.js` | 58 | 掲示板の画面 |

## ★この日の失敗（動画の分）

- **自分が付けた札が嘘だった。** 掲示板に `LINE×` と出していたが、掲示板は知らせに送らない。
  連絡帳のコードを持ってきたときに、**言葉まで一緒に持ってきてしまった。**
  → **他の画面から持ってくるときは、コードだけでなく【その画面で正しいか】を読み直す。**
    特に「◯◯できません」と言い切る表示。
- **壊れた行しか無いとき、中身の無い枠だけが残って余白ができていた。**
  `if (vids.length)` で枠を先に開いてから中身を詰めていたため。
  → **先に中身を作り、1つも作れなければ枠ごと出さない。**
- どちらも**台（ハーネス）が見つけた。** 目で読んだだけでは通していた。

## 保存の取りこぼしと、写真・動画のトグル 2026-09-07（後半）  <!-- readme-savevis-v1 -->

| marker | 何 | ファイル |
|---|---|---|
| `renraku-save-before-v1` | 「LINEで送る」「印刷」の前に保存する | `templates/renraku.html` |
| `renraku-line-vis-media-v1` | 表示項目のトグルを写真・動画にも効かせる | `app.py` `templates/renraku.html` |
| `renraku-line-vis-media-v2` | 送るもの・送らないものを**両方**見せる | `templates/renraku.html` |

### 保存の取りこぼし

「LINEで送る」も「印刷」も、中身は**保存したものから作る**。なのに保存せずに押せた。
書いたばかりの内容が入らないまま送られる・刷られる。
（「文章をコピー」だけは `renraku-line-items-v1` で先に保存していた。同じ手当てを広げた。）

**★印刷だけ作りが違う。**

    保存を待ってから window.open  →  押した操作から離れる
                                  →  iPhone/Safari がポップアップとして止める

なので **先に空のタブを開いてから保存する。** タブには「保存しています…」と出し、
保存できてから `w.location.href` に行き先を入れる。保存に失敗したらタブを閉じる
（白い画面を残さない）。すでにポップアップが止められていたときは、保存だけ済ませて
「**保存は済んでいます**」と添えて知らせる。
→ **2026-09-07 に実機（iPhone）でタブが開くことを確かめた。**

**★「まとめて印刷」(`rkPrintAll`) は触っていない。理由を忘れないこと。**
あれは一覧の画面にあり、そこでは保存バーが隠れている＝書きかけの入力が無い。
しかも **`RK_PATIENT` は一覧に戻っても残る。**
ここで保存すると**開いてもいない利用者の連絡帳を勝手に書く。** assert で固定した。

### 表示項目のトグルが写真・動画に効いていなかった

トグルは**利用者ごと**に設定できる。「この方は写真をご家族に送らないでほしい」を
**印刷では守れていて、LINEでは守れていなかった。** 設定した人の意図と逆。

原因：`_visible` を見ていたのは文章を作る `_renraku_to_line_text` だけ。
写真と動画は `api_renraku_line_send` が**画面から受け取ったまま**送っていた。

**HIROさん判断＝B案**（Aはトグルを見るだけ／Bはそれに加えて出どころも直す）:

1. 送る写真・動画を **`_renraku_fetch_for_line` で読んだ保存済みの中身**から取る。
   `data.get('image_urls')` `data.get('video_urls')` は**もう読まない**（assert で固定）。
   → 文章と同じ出どころになった。**受け取る口が1つ**なので、食い違いが起きない。
2. そのうえで `_renraku_visible_for` を見る。オフなら送らない。
3. `api_renraku_line_preview` にも**同じ決まり**を入れた。
   ここを揃えないと「3枚と出ていたのに1枚も届かない」が起きる。

**★読めなかったときの倒し方が、文章とは【逆】。**

| | 読めなかったら | なぜ |
|---|---|---|
| 文章のトグル | **全表示**に倒す | 項目が急に消えて驚かせないため（`_renraku_visible_for` のコメント） |
| 写真・動画 | **送らない**に倒す | 送ってよいか分からないまま送るほうが取り返しがつかない |

向きが違うのは意図的。**片方に揃えないこと。**

### v2 ＝ 送るものと送らないものは別の話

v1 は「送らないもの」があると「送るもの」を消していた。

    動画がオフ → 「動画は送りません。」だけ
                → 写真が送られることが分からない   ← HIROさんが見つけた

送るものを上、送らないものを下に、**両方出す**。

    文章と写真 1枚を一緒に送ります。
    この利用者は「動画」が表示項目でオフのため、動画は送りません。

送るものが無くても、送らないものがあるときは「**文章だけを送ります。**」と出す。
何も出ないと、文章まで止まったのかと思わせるため。
...
- **assert の数をまた頭で数えて2回外した。** 前半で「当ててから数える」と書いた直後に、である。
  - `renraku-save-before-v1` の印を4と書いた。実際は**3**。
  - `_renraku_shared_items(supabase, f_code, note_date)` を1と書いた。
    **定義の行にも当たって2。** 前半で `_renraku_visible_for` の定義行で
    同じことをやっているのに、また同じところで転んだ。
  → **「触っていないこと」を見る assert は、数を書かずに【前と後で同じ】で比べる。**
    `ap_b.count(k) == ap.count(k)` なら、数を知らなくても書ける。
- **`_top_block` を別のパッチに写すとき、`"\n"` が `"\\n"` になっていた。**
  それでも**落ちずに動いた**。JSの文字列（`confirm('...\nよろしいですか？')`）の中に
  **本物の `\n` という2文字**があって、たまたま当たっていたため。
  → **切り出しの道具は、写したら1回だけ「切れた中身」を目で見る。**
    動いたことは、正しく切れたことの証明にならない。
- **v1 で、警告を出すと「何が送られるか」を消していた。**
  送るものと送らないものを1つの場所に押し込んだのが原因。
  → **「何が起きるか」と「何が起きないか」は別の行に置く。** 片方でもう片方を上書きしない。

## 勤怠の合計が社労士さんの検算と合わなかった件 2026-09-07  <!-- readme-pay-v1 -->

社労士さんから「何度計算しても合計時間が提出されたデータと合わない」と言われた。
本番の実データを読んで原因を特定した。**計算そのものは合っていた。**
合わないのは**出力の見せ方**。社労士さんは 退勤−出勤−休憩 で検算していた。

marker: `pay-verify-v1`（`app.py`）／渡したもの: シンプル勤怠 CSV
締日は月末（`closing_day: 0`）。期間は正しかった。

### 原因① 秒（毎日1分ずつ効く）

出勤・退勤は `%H:%M` までしか出していないのに、実働は**秒まで見て**計算している
（`int((at - cur_in).total_seconds() // 60)`）。
出勤の秒 > 退勤の秒 の日は1分短くなる。**だいたい半分の日で起きる。**

    2026年8月の本番: 打刻 326件のうち 289件(88.7%) に秒が付いていた
    99日中57日でずれ、6名合計 57分

### ★原因② 中抜け（こちらが本命）

一度退勤して戻った日。CSVには**最初の出勤と最後の退勤しか出ず**、
間の空き時間はどこにも出ない。休憩(分) にも入らない。
**★しかも打刻異常の印が付かない。** CSVを見ただけでは気づけない。

    宇佐美友理さん 2026-08-21（実データ）
      in 08:21:52 → break 13:41:05〜14:12:24 → out 15:11:02
                    ↓ 中抜け 2時間43分
                    in 17:54:52 → out 20:24:40
      CSV: 出勤 08:21 / 退勤 20:24 / 休憩 31分 / 実働 527分
      見た目の引き算: 692分 → ★165分(2時間45分) 多く出る

打刻がそろっていない日（実働が空・合計に入らない日）は**8月はゼロ**だった。
社労士さんとの差は合計 222分(3時間42分)。うち165分がこの1日。

### 何を足したか

| 列 | 中身 |
|---|---|
| 途中退出(分) | `gap_min` … 退勤してから、また出勤するまで |
| 在席(分) | `stay_min` … 勤務時間帯の合計（休憩を含む） |

★**在席 − 休憩 = 実働** が【いつでもぴったり】成り立つ。社労士さんはこの1本で検算できる。
休憩が複数回でも、中抜けが複数回でも、秒がどう付いていても一致する。
**合計行でも成り立つ**（合計行に休憩・途中退出・在席の合計を出した）。

あわせて 出勤・退勤を**秒まで**表示（`_tc_fmt_time_jst_sec`）、
合計行に「打刻異常 N日（実働は合計に入っていません）」、
実働が空の日の打刻異常欄を「⚠合計に入っていません」で始まるようにした。

★列は**右端**に足した。中身が変わったのは 出勤・退勤 だけ（`09:00` → `09:00:50`）。
**社労士さんの取り込みが時刻の形を見ている場合は影響が出る。先に伝えること。**

### ★実働の数字は1つも変えていない。どう確かめたか

給与に直結するので、変えていないことを**機械で**確かめた。

- でたらめな打刻 **3000通り**で、直す前と後の
  実働・打刻異常・印・休憩・出勤・退勤が**すべて同じ**になること（`h_payverify.py`）
- **300か月分**のでたらめなデータで、合計行の
  「在席の合計 − 休憩の合計 = 実働の合計」が外れないこと

台: `h_payverify.py` 45件。

### 1分未満の扱い（★社労士さん待ち・未決）

いまは**区間ごとに切り捨て**（`// 60`）＝すでに1分未満は捨てている。
もう一つの形として「**打刻時刻の秒を先に捨ててから計算する**」があり、
そちらなら社労士さんの検算と一致するが、**支払う時間が増える**（8月で6名合計 +57分）。
しかも**それでは中抜けの問題は解決しない**（8/21は529分にしかならない）。

★労働時間の端数処理は**賃金の支払いに関わる論点**なので、こちらで決めない。
社労士さんに「1分未満はどう扱うのが正しいか」を聞き、指示どおりに直す。
説明文（`kintai_setsumei_202608.md`）の末尾でその質問をしている。

### 触っていないところ

勤怠集計表(グリッド)のCSV・PDF、給与計算の出力、`_pay_compute_staff`（残業・深夜・法定休日）。
まずシンプル勤怠だけ直して、社労士さんに見ていただく。

## ★この日の失敗（勤怠の分）

- **本番を調べるSQLに穴があった。** 職員ごとの合計を出すクエリで、
  「出勤1回・退勤1回」の日しか足しておらず、**宇佐美さんの8/21がまるごと抜けていた**。
  さらに休憩も `n_bs = 1 and n_be = 1` の日しか引いていない（休憩2回の日は0で引く）。
  → その数字をHIROさんに見せてしまった。**訂正した。**
  → **調べるためのSQLでも、出した数字を人に渡すなら本番と同じ条件で書く。**
    簡略化するなら「この数字は◯◯を含まない」と一緒に渡す。
- **Supabase の SQL Editor は【最後のクエリの結果しか返さない】。**
  診断を5本並べて渡したら、5本目しか返ってこなかった。
  → **診断は1本にまとめる。** 種類の列を付けて `union all` で縦に積む。
- （良かったこと）`assert` が2回止めてくれた。印の数を4と書いて実際は3、
  `_renraku_shared_items(...)` を1と書いて**定義の行にも当たって2**。
  → 「触っていないこと」の検査は**数を書かず、前と後で同じ**で比べる。

## ★残っていること（2026-09-07 の時点）

**本番へ出していないもの → ★同じ日の夜に4本とも本番へ出した（`3002b3f`）。**
下の「後半その2」を見ること。この表の4本は、もう本番に入っている。
...
- 動画: `.mov` が出てきたときの案内（iPhone 設定→カメラ→フォーマット→互換性優先）
- `line_friends.patient_id` だけ uuid、連絡帳側は text。移行するかどうか
- Google Workspace の解約そのもの（HIROさんが「サブスクリプションを解約」を押す段階）
- Google Play 開発者アカウントの持ち主（未確認）
- Apple: `DEVELOPMENT_TEAM` を差し替え → App ID登録＋Push有効化 → APNs鍵 → 実機 → Unlisted/Custom App
...
# 2026-09-07（月）後半その2 — 4本を本番へ出し、社労士さんへ送る形にした日  <!-- readme-20260907-2 -->

## 本番に出した（`3002b3f`）

`tasukaru-dev` の4本を `--no-ff` で `tasukaru` へ。**DBの変更なし。**
`renraku-save-before-v1` / `renraku-line-vis-media-v1` / `-v2` / `pay-verify-v1`。

## ★台（`h_*.py`）が1本も残っていなかった

引き継ぎ書に13本挙げてあった台が、`~/dev/kaigo-ai-app/` に**1つも無かった。**
前のセッションの作業場に置いてあって、そこが消えると一緒に消える。
＝「新しいパッチを当てたら全部流し直す」という決めごとが、**守れない状態**だった。
社労士さんへの説明文（`kintai_setsumei_202608.md`）も同じ理由で消えていた。

→ **台はリポジトリに入れる。** `h_payverify.py` を作り直してコミットした。
→ 作り直した台は **`app.py` と `app.py.bak` をその場で読んで関数を切り出す。**
　 写しを持たないので、`app.py` を直したあとに流し直せば、必ず今の中身を見る。
　 `app.py.bak` が pay-verify-v1 より前のものでなくなったら、前後比較(A)だけ自動で飛ばす。
　 `cd ~/dev/kaigo-ai-app && python3 h_payverify.py` → **25件OK**。
→ 切り出しの道具（`_top_block`）は、写したら1回は切れた中身を見る、と決めてあった。
　 目で見られないので、台の中で「頭が `def _tc_compute_day(punches):`」
　 「尻が `}` で終わる」「`\ndef ` を飲み込んでいない」を assert している。
→ わざと `stay_min` を1分ずらした `app.py` で流すと **B・C・D が7件まとめて落ちる**ことも確かめた。

## ★確認は DEV では取れなかった（本番の実データで取れた）

DEVには打刻が1件も無い。`_pay_fill_all_days` は
「打刻も休み登録も1つも無い職員には日を足さない」ので、
シンプル勤怠CSVは**合計行だけ**になる。これは正しい振る舞い（空欄の紙を増やさないため）。
勤怠集計表(グリッド)のCSVのほうは日が並ぶが、登録の無い日を「公休」と書く（後述）。

→ **先に本番へ出して、本番の8月を落として確かめた**（落とすだけ＝読むだけ）。

| 見たこと | 結果 |
|---|---|
| 在席 − 休憩 = 実働（実働のある**100日**） | **外れゼロ** |
| 【合計】行7名（日の行の再集計とも突き合わせ） | **全員一致** |
| 秒 | 出勤・退勤200件のうち **165件が `:00` 以外** |
| 打刻異常の日 | **ゼロ**（説明文の記述どおり） |
| 宇佐美友理さん 8/21 | `08:21:52 / 20:24:40 / 31 / 527 / 163 / 558` |

★**222分の内訳が確定した。二重計上は無かった。**

```
秒だけのずれ 57分
  （岸本朋子14・岸本洋幸13・加藤鮎美10・江良勇矢9・宇佐美友理6・坂野有希子5）
＋ 宇佐美友理さん 8/21 の 165分（中抜け163 ＋ 秒2）
＝ 222分
```

宇佐美さんの「6分」は**8/21以外の6日ぶん**で、8/21の2分はそちらに入っていない。
だから 57 + 165 で足して重ならない。

## ★管理者の打刻編集・追加は、秒を消す

`_tc_jst_to_utc_iso(date_str, time_str)` は `"HH:MM"` しか受け取らず、秒を `0` で作る
（画面の入力も `<input type="time">`）。
**管理者が打刻を1回直すと、その日の秒が消えて実働が最大1分動く。**
秒を見せるようにした以上、社労士さんの端数処理の話と地続き。**直すかどうかは未決。**

## ★`/timecard` の打刻は「端末が登録されている施設」に入る

`timecard_punch` は `f_code = dev.get("facility_code")`。**ログイン中の施設ではない。**
本番に登録された端末で押すと、**本番に打刻が入る。**
DEVでデータを作るつもりで踏むと「本番は読むだけ」を破る。
DEVに作るなら 管理者MENU ＞ 勤怠集計 ＞ 各日の鉛筆 ＞ 打刻を追加
（`admin_timecard_add` は `session["f_code"]` なので、ログイン中の施設に入る）。

## 社労士さんへ送ったもの

- `kintai_setsumei_202608_v2.md` と `kintai_setsumei_202608_v2.pdf`（A4・3ページ）
- 本番の `kintai_simple_202608.csv`（新しい形）

**前夜に送った説明文の数字は、1つも間違っていなかった。** 直したのは表現だけ。

| 直した所 | 前 | 後 |
|---|---|---|
| 反映時期 | 「次の月分から反映されます」 | **「本日、反映済み」**（実際にそうした） |
| 秒のずれ幅 | 「1日あたり最大1分」 | 「**1回の出勤〜退勤ごとに**最大1分」（中抜けの日は回数ぶん） |
| 222分の内訳 | 「秒57分 ＋ 中抜け165分」 | 165分の中身は「中抜け163 ＋ 秒2」と明記。57分は8/21を除くと注記 |
| 同じ打刻の例が2か所で違う数字 | §2は479分・§7は539分 | §7に「休憩を引く前の数字」と添えた |

PDFは HTML を書いて Chromium の `--print-to-pdf` で作った（フォントは Noto Sans CJK JP）。

## 休憩の運用（HIROさん説明）

- **日曜の1日型デイは、休憩中も利用者と一緒に食事をして介助することがあるので、
  休憩なしとして数えている。**
  実データもそのとおりで、岸本洋幸さん以外の「8時間超で休憩0分」の日は
  **例外なく日曜だけ**だった。平日は全員休憩がある。
  土曜や短時間の日の休憩0は、そもそも実働3時間前後で休憩の要らない日。
- 岸本洋幸さん（HIROさん本人）は23日勤務すべて休憩0分。
  **社労士さんも承知している**ので、説明文には書かない。

## ★この日の失敗（その2・次に読む自分へ）

| 失敗 | 学び |
|---|---|
| **台と説明文を作業場に置いたまま、git に入れていなかった。** だから消えた | **次に読む自分が使うものは、リポジトリに入れる。** 作業場は消える前提で扱う |
| **確認をDEVで取ろうとして往復をむだにした。** DEVには打刻が無く、原理的に取れなかった | **確かめたい性質が「実データにしか無い」ものかを先に考える。** 秒と中抜けは本物の打刻にしか出ない。DEVで作れるのは「列が増えたこと」までだった |
| **DEVで打刻を作る手順を渡す前に、`/timecard` がどの施設に書くかを読んでいなかった。** 端末が本番のものなら本番に書いていた | **「書き込む先」を決めているのがログインか端末かトークンか、手順を渡す前に読む** |
| 台を作り直したとき、**期待値を頭で計算して2つ外した**（見た目の引き算を秒つきで引いて164分／中抜け2回を切り捨てずに270分） | **当ててから数える、は数式にも効く。** とくに「人が見ている数字」は、人が見ている形（時:分）で計算する |

## 残っていること（2026-09-07 の終わりの時点）
...
- 管理者の打刻編集・追加で秒が消える件（社労士さんの端数処理の答え待ち）
- 勤怠集計表(グリッド)のCSVが、登録の無い日を「公休」と書く。
  シンプル勤怠と画面は「決めつけずに空欄」に揃えたが、こちらは昔のまま
- `video_hidden` はトグルだけを見るので、その日に動画が1本も無くても
  「動画は送りません」と出る。許容か、動画がある日だけ出すか

---

# 2026-09-08（火）送迎を3本・連絡帳を1本／通数の思い込みを1つ直した日  <!-- readme-20260908 -->

DEVまで入れたもの（本番へはまだ）。

| marker | 中身 | DB |
|---|---|---|
| `soge-past-admin-v2` | 過ぎた日の歯止めが、日付の【形】を見ていなかった | — |
| `soge-date-driver-v1` | 今日だけの配車で運転手を変えたら当日に届く | — |
| `soge-back-plan-v1` | 迎えを【到着時刻から逆算】する | **列1つ** |
| `soge-run-plan-edit-v1` | 運行画面でも到着予定時刻を直せる | — |
| `line-multi-patient-v1` | 1つのLINEアカウントに複数の利用者 | **表1つ** |

SQLは2本とも DEV・本番の両方で流し済み。本番へマージするときにSQL待ちは無い。

## 迎えを到着時刻から逆算する（`soge-back-plan-v1`）

★HIROさんの決め：**送りは出るのが決まっているので前向き、迎えは着くのが
決まっているので到着から逆算。** 余った時間は「迎えの手前」に待機として出る。

    中間便 12:00発 / 到着 13:30、送り2人・迎え2人、各区間10分、乗降2分

    12:00  出発
    12:10  A様（送り）      ← 前向き。これまでどおり
    12:22  B様（送り）
            ┈┈ 待機 32分 ┈┈
    13:06  C様（迎え）      ← 13:30 から逆算
    13:18  D様（迎え）
    13:30  事業所 着        ← ★ちょうど

前向きのままだと 12:34 / 12:46 に着き、C様・D様を**44分と32分ぶん早く待たせて**いた。

- 逆算するかどうかは**施設ごとのトグル**（`soge_settings.back_plan`）。
  効かせるのは迎えのある便だけ。送り便は前向きのまま
- 「到着」は**便ごとの設定**（`trips` のJSONの中。SQLは要らない）
- **間に合わない日は前向きに倒して警告**。現場はいつもどおり動かし、無理な日だけ見せる

★作りとして都合がよかったのは、`_soge_stops_of` がもともと**送り→迎えの順**で
立ち寄りを返していたこと（`_soge_order_stops` がその順序を守る）。
だから**並びの真ん中で切って、前半は前向き・後半は逆算**にするだけで届いた。

★**帰りの区間は `legs` に入っていない。** `legs` は行きの n 区間だけ。
走行時間の合計（n+1区間ぶん）から引いて出す。ここを 0 にして台を流すと
**8件まとめて落ちる**ことを確かめてある。

## 運転手が当日に届かなかった件（`soge-date-driver-v1`）

「直前だと反映しない」ではなく、**その日の便ができたあとは【いつでも】届かなかった。**
理由が2つ重なっていた。

  (a) 動き出している日は `_soge_merge_day` が `reason="touched"` で何も当てない
  (b) 動き出していなくても、9/2 に入れたこの歯止めで止まる

        if day and (day.get("driver_name") or "").strip():
            head["driver_name"] = day.get("driver_name")

(b) は「朝に替えた運転手が、画面を開き直すと戻る」を止めた措置で、**それ自体は要る。**
だから (b) は触らず、**配車を保存したときに直接当てる**ようにした。

★**黙って戻さない。** 運行画面で立てた代理が保存で戻ることがあるので、
変わったら「◯◯便：山田 → 佐藤」と保存の返事に出す。
記録表には運転手の名前が必須なので、黙って変わるのがいちばん困る。

## 運行画面で到着予定時刻を直せるようにした（`soge-run-plan-edit-v1`）

いま直せるのは**打刻（`arrived_at`）だけ**で、**予定（`planned_at`）**は
配車表からしか直せなかった。同じ口（`/api/soge/run/stop`）に `plan` を足した。

★**打刻と予定を混ぜない。** 「時刻を手で」は打刻、「予定を直す」は予定。
台で「予定を送っても打刻に触らない／打刻を送っても予定に触らない」を固定した。

★**予定は貯めない**（電波が戻ってから送り直す仕掛けには乗せない）。
予定は実績ではないので、入らなかったらその場で言えば足りる。
貯めると、あとから古い予定が入って現場を惑わせる。

## 1つのLINEアカウントに複数の利用者（`line-multi-patient-v1`）

娘様がご両親2人ぶんの連絡帳を受け取れるようにした。
紐付けだけを `line_friend_patients`（1行＝1つの関係）に切り出した。

★**古い列（`line_friends.patient_id` / `status`）は残して、そろえ続ける。**
  `status` は1人でも紐付いていれば `linked`（画面の出し分けがそのまま動く）、
  `patient_id` には1人目を入れる。**この1人目が保険。**
  新しい表が読めないときはここへ倒れ、**1人目のぶんは必ず届く。**
  「誰にも届かない」だけは起こさない。

★**「0件」と「読めなかった」を分ける。** 読めないのを0件と読むと、
紐付いているのに「未紐付け」と出てしまう。

★画面は**紐付け済みでも検索欄を残す。** 前は紐付くと検索欄が消えていたので、
2人目を足す場所が無かった。解除は**利用者ごと**。

★**まだ入れていないこと**：LINEの文章に**利用者名が入っていない**
（`_renraku_to_line_text` は `patient_name` を受け取っているのに使っていない）。
2人ぶん届くと、同じ形の連絡が並んで見分けがつかない。
HIROさんの判断で**今回は入れない**。入れるなら1か所だけ。

## ★本番で500が出ていた件 → 回線の巻き添えだった

9/7 のスクショで `/api/records/unread_count` が500を返していた。
URLの長さ（`in_` に id を全部載せる作り）を疑ったが、**データを見て否定された。**

    must_read 27件 / records 総数 4,866件 / records.id は【整数】2〜4桁

27件ならURLは150文字ほど。全く足りている。3回叩き直して**3回とも200**。
同じ並びに `ERR_NETWORK_CHANGED` と `ERR_NAME_NOT_RESOLVED` があり、
**回線が切り替わっている最中**だった。

★**思い込みで直しに入る前に、データを1回見る。** 見なければ、当たっていない
場所を直して「直った気」になっていた。

★ただし `.in_(` は **56か所**あり、`_fetch_all_paginated` を通しているのは
掲示板の4か所だけ。id が uuid の表で件数が増えると、いつか当たる形ではある。

## ★LINEのメッセージ通数（調べ直して訂正した）

HIROさんの「一斉配信でなければ消費しないのでは」に対し、**僕は先に断定していた。**
公式を読み直した結果：

- **カウントされる**：Push / Multicast / Broadcast / Narrowcast（**1対1のプッシュも**）
- **無料**：応答メッセージ（Reply）・LINEチャット・あいさつメッセージ・届かなかったぶん
- 数え方は **送信リクエストの数 × 宛先の人数**

★**僕が言い過ぎていた所**：吹き出しの数は関係ない。
「3つの吹き出しを5人に送っても 1リクエスト×5人＝5通」。
TASUKARU は5件ずつまとめて送るので、**文章＋写真4枚までなら1人1通**。

無料プランは月200通。**娘様2人にすると、その利用者ぶんが2倍**になる。
使用量は LINE公式アカウント管理画面 ＞ 分析 ＞ メッセージ通数 で見られる。

## ★実績集計の「午前・午後が分からない来所」は、直しようがない

脇島由富さんの8月の金曜3件。**利用曜日の登録漏れではなく、金曜の利用が終わった**だけ。

`patient_visit_days` は `half_start_date` / `full_start_date`（開始日）を持つのに
**終了日も履歴も持たない。** だから曜日をやめた瞬間、過去の来所がさかのぼって
「未設定」になる。しかもその3件は**曜日別の実績からも抜ける**。

誰かの利用曜日が変わるたびに同じことが起きる。直し方は2つ。

- **A（安い）** 言葉を変える。「午前・午後が分からない来所」→
  「いまの利用曜日に無い曜日の来所」。警告ではなく事実として出す
- **B（重い）** 終了日か履歴を持たせ、過去の月を当時の設定で判定する

**未決。** この表を行政や外部に出すならBが要る。

## ★この日の失敗（次に読む自分へ）

| 失敗 | 学び |
|---|---|
| **「触っていない所」の鍵が、自分の書いたコメントに当たって落ちた**（今日だけで3回）。README に3回書いてある転び方 | **仕組みで止めた。** パッチの頭で「置き換えの前後で鍵の数が同じ」を assert する。目印には元の行を含めるので「新しい文に出てこないこと」では厳しすぎて必ず落ちる |
| **数を頭で数えて4回外した**（`return False` の本数／`.strftime` の数／`trip.get("arrive")` の数／入れ子の `def`） | **当ててから数える。** 入れ子の `def` は字下げされていて `"\ndef "` には当たらない（0が正しい） |
| **凍結する関数を間違えた。** `_soge_merge_day` は呼び出しを1行足すので凍結できないのに FROZEN に入れた | **凍結は「1文字も変えない関数」だけ。** 触る関数は、中の歯止めを名指しで見る |
| **本番の500を、データを見る前にURL長だと断定しかけた** | **直しに入る前にデータを1回見る**（上記） |
| **LINEの通数を、調べる前に断定して言い過ぎた。** HIROさんの指摘で調べ直して訂正した | **相手の事業のお金に関わる話は、断定の前に一次情報を見る。** 指摘されたら、まず調べ直す |
| （良かったこと）**型を推測せずに `information_schema` を1回読んだ。** `line_friends.patient_id` は uuid、`renraku_notes.patient_id` は text | 決めごとどおり。おかげで本番の移行が一発で通った |

## 残っていること（2026-09-08 の終わりの時点）

**DEVで実機確認したいこと**

- 運転手：今日だけの配車で変えて保存 →「運転手も当日の運行表に反映しました」が出るか。**打刻を入れてから**でも届くか
- 逆算：送迎設定に「到着」欄とトグルが出るか。ONで中間便に到着を入れると、**送りは前向き・迎えが後ろに寄る**か
- 予定時刻：一覧の「予定 08:49」を押して直せるか。「時刻を手で」（打刻）と混ざっていないか
- 複数紐付け：1人の友だちに2人を紐付け → 両方の連絡帳が届くか。片方だけ解除できるか
...
- 配車表の手入力（`soge-plan-manual-v1`）にも**全角の穴**がある。
  `isascii()` が無いので「０８:３０」が通る。1行で塞げる
- 実績集計の「午前・午後が分からない来所」（上記A/B）
- 1つのLINEアカウントは**1人の利用者にしか紐付かない**という制限は解けたが、
  **文章に利用者名が入っていない**ままなので、2人ぶん届くと見分けにくい
- 勤怠集計表(グリッド)のCSVが、登録の無い日を「公休」と書く
- `video_hidden` はトグルだけを見るので、動画0本の日でも「動画は送りません」と出る
# 2026-09-08（火）後半 — 本番へ出し、そこで初めて見えた不具合を2つ直した日  <!-- readme-20260908-2 -->

朝までにDEVへ入れた7本を本番へ出した。**そのあと本番の画面を見て、
DEVでは一生見つからない不具合が1つ出た。** それを直した記録。

## 本番へ出したもの

| 本番のコミット | 中身 |
|---|---|
| `109df77` | 送迎3本・連絡帳1本・過ぎた日の歯止め・README（DEVの7本まとめて） |
| `c90e2be` | `soge-nogeo-leg-v1` … 送り便の1人目が出発時刻と同じになる |
| `ce9abfa` | `soge-wait-show-v1` … 逆算した便の待機時間を配車表に出す |

## ★送り便の1人目が、出発時刻と同じ時刻に着くことになっていた（`soge-nogeo-leg-v1`）

本番の運行画面で HIROさんが見つけた。

    送り便 16:30 出発
    1 小倉様      予定 16:30   ← ★出発と同じ
    3 佐々木様    予定 16:43
    4 河野様      予定 16:51

走行時間の区間ごとの配列を見たら **`[0, 9, 4.15, 6.15, 2.32]`**。先頭が 0分。
他の便にも `[3.32, …, 0, 9.35, …]` のように 0 が混ざっていた。

### 穴は3つ重なっていた

**① 座標が無い人の区間が 0分 になる**

`_soge_drive_detail` は、座標が無い人を**そもそもGoogleへの経路に入れない**。
入れない人の区間は `0.0` のまま残る（コメントにもそう書いてあった）。
0分 ＝「1つ前の場所から一瞬で着く」。1人目なら**出発時刻と同じ**、
途中の人なら**前の人と同じ時刻**が出る。

**② 座標を付けても、永久に直らない**

走行時間のキャッシュの鍵（`route_hash`）が **「施設の住所＋立ち寄り順」だけ**だった。
あとから座標を付けても鍵が変わらないので、**古い 0分 を返し続ける**。
これがいちばん質が悪い。「住所を直したのに画面が直らない」という形で出る。

**③ 住所→座標の変換を走らせるボタンが、画面のどこにも無かった**

`/api/soge/geocode` という口は前から作ってあるのに、**置き忘れていた。**
つまり、利用者を追加しても座標が付く道が無い。①の引き金がここにあった。

### 直し

- 座標が無い人の区間を、**分かっている区間の平均**で埋める。
  ★**同じ住所のご夫婦などは座標があるので触らない。** 本当に0分でよい区間は0分のまま
- **鍵に「座標の有無」を混ぜる。** 座標が付いた時点で鍵が変わり、次に開いたときに1回だけ取り直す
- 座標が無い人を配車表に**名前で**出す＋**「座標を取り直す」ボタン**を警告の下に置く。
  保存済みの配車表（ふだん見るほう）には**1文字も出ていなかった**ので、そちらにも出す

到着予定時刻の計算そのもの（`_soge_planned_times`）と逆算（`soge-back-plan-v1`）は
**1行も触っていない。** 台は `h_nogeo.py`（35件）。

### 未解明のまま残したこと

座標が無い人を数えるSQLでは **6名** 出たのに、変換を走らせたら `geocoded: 0 / cached: 75`
（＝全員そろっている）と返り、そのあと同じ6名を引き直したら**全員「座標あり」**だった。
辻褄が合っていない。**SQLの絞り込みと `patient_active_on` の条件を揃えていなかった**のが
原因だと思うが、確かめていない。また誰かの座標が消えたら、ここから追う。

## 逆算した便の待機時間を配車表に出す（`soge-wait-show-v1`）

計算はできていて `wait_minutes` として配車表に渡っていたのに、**画面に出していなかった。**
車の見出しに「**待機 25分**」（緑）、逆算が間に合わなかった車に
「**到着に間に合いません**」（赤）を出す。画面だけの直し。

★**所要時間とまったく同じ扱いにした。組み直した直後だけ出て、保存すると消える。**
保存済みの配車表は、保存したときの時刻を並べているだけで走行時間を持っていない
（だから所要時間も出ていない）。そこに待機だけ出すと、**古い数字を今の数字の顔で見せる**。
並びを1人入れ替えれば待機は変わるのに数字は変わらない、という嘘になる。

いつ開いても出したくなったときの選択肢（**未決。使ってみて決める**）:

| | やり方 | 代償 |
|---|---|---|
| A | 開くたびに車ごとの走行時間を引き直す | 表示が遅くなる。手で並べ替えた便はGoogleに聞き直す（課金） |
| B | 保存に列を1つ足して残す | SQL1本。組み直さずに保存すると古い待機が残る |

## ★この日の後半の学び

| 出来事 | 学び |
|---|---|
| **DEVでは絶対に見つからなかった。** DEVには本番の住所も、0分が焼き付いたキャッシュも無い | **データに依る不具合は、本番の画面を見るまで見えない。** 出したら必ず本番で1回見る |
| **「口はあるのにボタンが無い」**。APIを作った日に画面へ置き忘れると、誰も気づかない | 口を足した日に、**押す場所も同じ日に置く**。置けないなら README に「口だけある」と書く |
| **キャッシュの鍵に、結果を変える入力が入っていなかった。** 座標の有無で結果が変わるのに鍵は並び順だけ | **鍵には「結果を変えるもの」を全部入れる。** 入れ忘れると、直したのに直らない、という最悪の形で出る |
| **SQLの絞り込みを、アプリの条件と揃えなかった**（`patient_active_on` を再現しなかった）ので、食い違う数字を出した | **アプリの判定をSQLで真似るときは、関数を読んでから書く。** 数が合わないときは、まず自分の絞り込みを疑う |

## 残っていること（2026-09-08 の本当の終わり）

上の「DEVで実機確認したいこと」のうち、**運転手・逆算・予定時刻の3つは本番で確認済み**。
`soge-nogeo-leg-v1` の直りも本番で確認した（1人目が出発時刻から動いた）。

**まだ見ていない**

- **複数紐付け**：1人の友だちに2人を紐付け → 一覧に2人並ぶか、片方だけ解除できるか。
  ★連絡帳を**送らずに**足して、確かめたら外す
- 待機の表示（`soge-wait-show-v1`）を本番の画面で見る
...
- 配車表の手入力（`soge-plan-manual-v1`）の全角の穴（`isascii()` が無い。1行で塞げる）
- 実績集計の「午前・午後が分からない来所」（A：言葉を変える／B：終了日か履歴を持つ）
- 連絡帳の文章に利用者名が入っていない（2人ぶん届くと見分けにくい）
- 勤怠集計表(グリッド)のCSVが、登録の無い日を「公休」と書く
- `video_hidden` が、動画0本の日でも「動画は送りません」と出す
# 2026-09-08（火）夜 — 管理者MENUを作り直した日  <!-- readme-20260908-3 -->

画面の作りだけ。`app.py` は1行も触っていない。

| 本番のコミット | 中身 |
|---|---|
| `a2a9905` | 管理者MENUの整理まとめ（★件名が文字化けしている。下記） |

marker は5つ。

| marker | 中身 |
|---|---|
| `line-multi-ui-fix-v1` | LINE友だち管理で利用者名が1文字ずつ縦に並んだ |
| `admin-patients-top-v1` | 利用者一覧を一番上へ。新規登録をその中に畳んで入れる |
| `admin-list-fold-v1` | 名前の一覧を畳む。検索欄は出したまま |
| `admin-settings-subtabs-v1` | 設定の中に第2段のタブを4つ |
| `admin-tabs-v2` | 上のタブを3つ→5つ。入口を部類ごとに置き直す |

## いまの形

    利用者管理 │ 職員管理 │ データ │ 運営・契約 │ 設定

| タブ | 中身 |
|---|---|
| 利用者管理 | 利用者一覧（新規登録を内包）だけ |
| 職員管理 | 一番上にタイムカード（勤怠） |
| データ | 実績集計表 ／ 利用管理（予定・実績の確認） |
| 運営・契約 | 契約書・重要事項説明書 ／ BCP管理 |
| 設定 | 第2段タブ4つ（下表） |
...
| | 中身 |
|---|---|
| 事業所 | 施設情報 ／ 契約・お支払い ／ 管理者パスワード変更 ／ ログイン端末の管理 |
| 送迎 | 送迎設定 ／ 車両マスタ |
| LINE | LINE連携設定 ／ LINE友だち管理 |
| 記録と書類 | 目標期間 ／ AIカテゴリ ／ 書式マッピング ／ 読み上げ音声 ／ 履歴の表示件数 |

★HIROさんの決め:
- **新規登録と利用者一覧は1か所に集約する。** 一覧が一番上・開いた状態、登録はその中に畳む
- **名前がずらりと並ぶのは畳む。検索欄は出したまま。** 打つと自動で開き、空にすると畳む
  （閉じたまま絞り込むと「該当が無い」と誤解するため）
- **BCPだけ運営・契約へ。** 施設情報と契約・お支払いは設定→事業所に残す

## ★この画面で踏んだ地雷（次に触る自分へ）

**① `.btn` は共通CSSで `width:100%`（base.html）**

横並び（`display:flex`）の中にそのまま置くと、ボタンが幅を全部取り、
**隣の文字が1文字ずつ縦に折り返される。** 本番のLINE友だち管理で実際に出た。

    <div style="display:flex">
      <span style="flex:1 1 auto;min-width:0;">名前</span>   ← min-width:0 が要る
      <button class="btn" style="width:auto;flex:0 0 auto;">解除</button>
    </div>                        ↑ .btn を flex に置くときは必ずこの2つ

**② `toggleAcc()` は【インラインの `display`】しか見ない（static/admin.js）**

    const isOpen = body.style.display !== 'none' && body.style.display !== '';

CSSには `.accordion-body.open { display:block }` もあるが、**この関数は見ていない。**
既定で開いた状態にしたいときに `class="open"` を付けると、1回目に押しても
`isOpen` が false と読まれて「開く」処理が走り、**1回目が効かない**（2回押さないと閉じない）。
→ 開けておくなら `style="display:block;"` を直接書く。矢印も `style="transform:rotate(180deg);"`。

**③ `switchTab()` は `.tab-panel` を【全部】閉じる**

    document.querySelectorAll('.tab-panel').forEach(p => p.classList.remove('active'));

第2段のタブに同じ関数・同じクラスを使うと、**設定タブごと消える。**
→ `switchSubTab()` と `.subtab-*` に分けた。探す範囲も `#tab-settings` の中だけに絞った。

## ★置き場所を動かすパッチの型（今日4本書いて固まった）

- **文字は打ち直さない。** 目印の位置で切り出して、そのまま詰め直す。
  打ち直すと必ずどこかが変わる（空白・全角・スタイル）
- **タグの開きと閉じが釣り合っているかを assert する。**
  数そのものではなく `開き - 閉じ` の差を比べる。増やした入れ物のぶんは差が変わらない
- **Jinja の `{% %}` `{{ }}` の数も比べる。** ここがずれるとテンプレートごと落ちる
- **「どの箱がどのタブに入ったか」まで assert する。**
  数が合っていても、入れる先を間違えていれば意味が無い
- ★数を頭で数えない。当ててから数える（今日もまた2回外した）

## 本番のコミットの件名が文字化けした

`a2a9905` の件名が `ÿffffffffffffe7...` になっている。
**貼り付けのときに日本語のバイトが壊れた。** 中身のコードには影響していない。
直すには本番ブランチの履歴を書き換える（force push）ことになるので**直していない**。

    a2a9905 = 管理者MENUの整理を本番へ
              admin-patients-top-v1 / admin-settings-subtabs-v1 /
              admin-list-fold-v1 / admin-tabs-v2 / line-multi-ui-fix-v1

★次から、**マージの件名は短く・ASCIIで**渡すこと。長い日本語ほど貼り付けで壊れる。

## 残っていること（2026-09-08 の夜の時点）

**まだ見ていない**

- 待機の表示（`soge-wait-show-v1`）を本番の配車表で見る
- 管理者MENUの5タブを本番で通しで見る（特に **設定→事業所**。
  BCPが消えてログイン端末が増えている、いちばん動かした場所）

**未決**

- 待機を「いつ開いても出す」か（A: 開くたび引き直す／B: 保存に列を足す）。
  使ってみてから決める
- 緑のリンクを2列のタイルにするか（いまは1列のまま）
...
- 勤怠集計表(グリッド)のCSVが、登録の無い日を「公休」と書く
- `video_hidden` が、動画0本の日でも「動画は送りません」と出す
# 2026-09-08（火）深夜 — 「待機」を見ようとして、出ない理由が分かった日  <!-- readme-20260908-4 -->

| 本番のコミット | 中身 |
|---|---|
| `1574c89` | `soge-keep-times-v1` … 「組み直す」でも所要時間・待機・予定時刻を出す |

## ★配車表の時刻は、どこで出て、どこで保存されるのか（毎回混乱するので図で残す）

    ボタン            中で呼ぶもの                所要時間・待機・予定時刻
    ───────────────────────────────────────────────────────────
    ゼロから          soge_build_week             出る（前からここだけ）
    組み直す          soge_build_week_keep        ★前は【出なかった】→ 直した
    画面を開く        _soge_rows_view             出ない（保存済みを読むだけ）

    保存（_soge_plan_rows）
      → 手で入れた時刻【だけ】を stop_order に残す。
        ★自動で計算した時刻は保存しない。たまたま出た値が
          「決まり」として固まってしまうため。これは正しい決まり。

`soge_build_week_keep` は最後にこう書いてあった。
...
**「保存時に引き直す」は起きない。** 保存は自動の時刻を保存しないのだから、
引き直す場所がそもそも無かった。結果として、所要時間も待機も予定時刻も、
**「ゼロから」を押したときにしか出ていなかった**。そして「ゼロから」は
手で直した割り振りが全部消えるので、本番では押せない。＝実質どこにも出ていない。

直し（`soge-keep-times-v1`）: 組み直したその場で測る。

- ★**並びは触らない。** 保存済み／手で直した順のまま測る。
  `_soge_stops_of` を通すと並べ直してしまうので、ここでは通さない
- ★**手で入れた時刻は上書きしない**
- ★**保存の決まりは1文字も変えていない**。自動の時刻は今までどおり保存しない
- 費用は車ごとに走行時間を1回。押したときだけなので、画面を開くたびの負荷は増えない

台は `h_keeptimes.py`（28件）。

## ★DEVで実際に確かめた（2026-09-08 深夜・Claudeがブラウザで操作）

火曜・迎え便（08:30発／到着 09:30／青木さん1名）で「組み直す」を押した結果:

    車1 テスラ 運転手…            3分  待機 57分 ／ 席 1/7
      1 迎 青木 利夫                              09:27

- **`soge-keep-times-v1`** … `3分 ／ 席 1/7` が出た（前はここが「保存すると再計算」）
- **`soge-wait-show-v1`** … 緑の「待機 57分」
- **`soge-back-plan-v1`** … 到着 09:30 から逆算して 09:27（前向きなら 08:30）
- **`soge-addppl-type-v1`** … 中間便が0人でも「迎え／送り」が選べた
- **`soge-nogeo-leg-v1`** … 警告と「座標を取り直す」が出て、押すと
  「0名ぶん取れました。取れなかった方: …」（DEVの住所はダミーなので取れないのが正解）
- 管理者MENU … 5タブ／一覧が上・開いた状態／新規登録は畳み／設定の第2段タブ4つ

★DEVの設定を2つ変えてある: **逆算をON**、**迎え便の到着に 09:30**。
　中間便に**空の車が1台**残っている（「最後の1台は外せません」で消せなかった）。

## ★未決: 手で足した人が「組み直す」で消える

中間便に手で2人足して保存 → 「組み直す」を押したら
**「その曜日に来なくなった方を 2件 外しました」** で消えた。

`soge_build_week_keep` は **その曜日の対象者（`_soge_want_pairs`）を正**として、
対象でない人を外す。だから「その便の対象者ではない人」を手で足しても、
次の組み直しで必ず消える。**動きとしては正しい**が、現場では驚く。

- 逃げ道はある。**「登録のない方（名前を直接入力）」で足した人（guest）は外れない**
  （keep は `s.get("guest")` を明示的に守っている）
- 直すなら「手で足した人には印を付けて守る」だが、
  そうすると「来なくなった人が消えない」という逆の困りごとが出る
- **未決。** 実際に困ってから決める

## 残っていること（2026-09-08 の本当の最後）

**済み**

- 管理者MENUの5タブ（本番で確認）
- `soge-addppl-type-v1` の3点（本番で確認）
- 待機・逆算・組み直しの時刻（DEVで確認。本番は設定次第）

**まだ見ていない**

- 本番の配車表で待機を見る（**本番の送迎設定で逆算がONになっているかが先**）

**未決**

- 手で足した人が「組み直す」で消える（上記）
- 緑のリンクを2列のタイルにするか
...
`h_backplan.py` に **「`trip.get("arrive")` がちょうど9個あること」** という検査を書いていた。
今日の直しで11個になって落ちた。**数を数える検査は、直すたびに落ちるだけで何も守らない。**

「**予定時刻を出す呼び出しが、全部そろって到着を渡しているか**」を見る形に変えた。
これなら呼び出しが増えても、渡し忘れたときだけ落ちる。

★今日の教訓、3回目。**数を頭で数えない。数えるなら「意味のある性質」を数える。**

# 2026-09-10（木） — 解約時の書き出しと、料金の組み直し  <!-- readme-20260910 -->

| 本番のコミット | 中身 |
|---|---|
| `f73262e` | data-export v1/v2、export-busy v1/v2、export-names v1 |
| `e80c537` | plan-size-split v1、plan-staff-count v1、plan-staff-limit v1 |
| `b51fbac` | pricing-card-off v1 |

本番Stripeに **V3の価格28本（LIVE）** を作成済み。`/dev/stripe_check` で 28/28。

## 1. データの書き出し（解約時に持ち出せるようにする）

★介護記録には保存義務がある（厚労省令「完結の日から2年」、多くの自治体が条例で5年）。
**持ち出せないと、解約した事業所が法令違反になる。それは持ち出せない作りにした
こちらの責任になる。** 契約書に「解約時に書き出せます」と書く前に、これを作った。

管理者MENU → データ → 「データを書き出す（ZIP）」。83表をCSVにしてZIP1本。

    01_利用者/利用者（patients）.csv
    02_記録/日々の記録（records）.csv
    …13フォルダ／目録.csv／はじめにお読みください.txt

★**向きに注意すること。** プランの門は【通すものを名指し】＝締まる側へ倒す。
書き出しは逆で【出さないものだけ名指し】＝出る側へ倒す。漏れたときに困るのが、
あちらは「穴が開く」、こちらは「持ち出せない」だから。

★日本語のファイル名は app.py の使われ方から確かめて付けた。頭で訳すと間違える。

| 表 | 直訳すると | 実際は |
|---|---|---|
| `accounts` | アカウント | **勘定科目**（会計） |
| `journal_entries` | 日誌 | **仕訳** |
| `record_vas` | VAS | **痛みの程度**（部位・左右・0〜10） |

★写真と録音の実体は入らない（職員40名・5年で約118GB。ZIPにできる大きさではない）。
置き場所のURLは表の中にある。

### 書き出し中の案内（export-busy）

押しても画面が動かないと「壊れた」と思って何度も押される。
**書類出力（print_output.html）の .po-loading-overlay と同じ動き**を使う
（タスカル君が左から右へ歩く）。歩く速さ3秒・跳ね0.4秒・影絵にする指定まで同じ。
★あちらを直したらこちらも直すこと。

★**進み具合の％は出さない。** ZIPはサーバ側で作り終えてから一気に送られてくるので、
途中の割合は本当は分からない。分からないものを棒グラフにすると、止まって見えた
ときに「フリーズした」と読まれる。出すのは受け取ったMBだけ。

## 2. 料金の組み直し（人数と機能を切り離す）★案Aで確定

    これまで : プランを選ぶと人数の枠まで付いてきた
              スターター10名 / スタンダード20名 / プロ30名
    これから : プランは【機能の段】だけ。人数は全プラン共通で10名まで
              11名以上は5名ごと +2,000円（録音+15時間）

**なぜ直したか。** 職員8名の事業所が連絡帳を使いたいだけでプロ（30名ぶん）を
買わされていた。1人あたり3,100円。40名の事業所は720円。同じものを使って4.3倍。
営業の相手はほとんどが小規模なので、ここが売れない理由になっていた。
...
7つの契約期間すべてで同額。**これは口約束にせず、組み直す前の金額を
`.bak` から読んで28通り突き合わせる点検**を patch に入れてある。

機能の値段も変えていない。分解するとこうなる。

    スターター　　10名 × 400円 = 4,000円 ＋ 機能  1,980円 = 　5,980円
    スタンダード　10名 × 400円 = 4,000円 ＋ 機能  4,800円 = 　8,800円
    プロ　　　　　10名 × 400円 = 4,000円 ＋ 機能 12,800円 = 16,800円

機能分 1,980 / 4,800 / 12,800 は組み直しの**前と同じ**。人数分だけをそろえた。
営業では「値下げしたのではなく、余っていた人数枠を外しました」と言える。

### 実装（3本）

| marker | 中身 |
|---|---|
| `plan-size-split-v1` | 金額の組み直し。★`PRICE_KEY_VER` を V2→V3 |
| `plan-staff-count-v1` | 契約時に職員数を受け取り、追加5名を決済に積む |
| `plan-staff-limit-v1` | 契約人数を超えたときの案内（★止めない） |

★**`PRICE_KEY_VER` を上げる意味。** Stripeの価格は lookup_key で作り置きしてある。
版を上げないと、新しい料金表を出しながら**古い金額で課金する**。版を上げると
V3を作るまで決済は動かない。**止まるほうを選ぶ。間違った金額を取るより、
取れないほうがまし。**

★**決済の入り口は2つある。両方直すこと。**
`/api/stripe/create_checkout`（既存事業所）と `/api/onboard/create_checkout`
（LIFFの申込）。片方だけ直すと、そちらの穴から40名が10名ぶんの金額で契約できる。

★**契約人数（staff_limit）は施設の行に混ぜて書かない。** 別のUPDATEにする。
行に混ぜると、列がまだ無いときに施設の作成ごと失敗して
**「払ったのに使えない」**になる。書けなければログに残るだけで、契約は通す。

★**`staff_limit` が空（NULL）の事業所は【上限なし】。** 空欄を0と読むと、
明日から全事業所が上限オーバーの赤い帯になる。読めなかったときも上限なしへ倒す。
**数えられないことを理由に「超過」と言わない。**

★**契約人数を超えても登録は止めない。** 現場は人が増えた日に登録する。
そこで止めると仕事が止まる。お金の話で現場の手を止めない。案内は
「登録できません」ではなく「◯名ごとの追加をお申し込みください」。

## 3. この日に見つけて直した不具合

**開発者MENUの施設一覧が「0施設」**（本番だけ）。
`facilities` の14列一括SELECTに `youshiki_exclude_enabled` が入っているが、
**本番にだけその列が無かった**。列が1つ欠けるとSELECTがまるごと失敗する。
→ 本番に列を追加して解決。

★DEVと本番は **Supabaseが別プロジェクト**（DEV `otjevnmo…` / 本番 `abvglnkw…`）。
**DEVで列を足したら本番にも足すこと。**

★この一括SELECTは `except: pass` で理由を握りつぶしている。画面にもログにも
何も出ない。**まだ直していない。次に触るときの候補。**

**一括払いが値上げに見えていた**（/pricing）。
月額 ¥17,980 に取り消し線を引いて、その隣に総額 ¥518,400 を並べていた。
「17,980円が518,400円になる」と読める。取り消し線の相手を【月あたり】へ移した。

**カードの「職員10名まで」が直らなかった。**
職員数を40名にすると金額は変わるのに、カードの中は固定値のままだった。
人数と録音はもうプランと関係ないので、**カードから外して職員数の欄に1回だけ**出す。

**書き出しZIPの名前が消えていた。**（自分で壊した）
サーバは `施設名_日付.zip` と名前を付けているのに、ダウンロードを自前に
切り替えたときJS側で決め打ちにして捨てていた。Content-Disposition から取る形に直した。
★自前でダウンロードさせるときは、**サーバが付けた名前を必ず使う。**

## 4. 資料（6本を作り直し）

お客様向け：料金プランのご案内 / チラシ / 他社比較
社内用：料金設計まとめ / 詳細 / 比較の社内版

★比較資料に**比較として成り立っていない行**があった。「職員数が増えたとき」の表で、
10名のところがスターター（送迎表も連絡帳も無い）になっていた。相手は
記録ソフト＋AI議事録の全部入り。**プロでそろえ**、行の名前も「TASUKARU プロ」にした。

★案A・案Bの比較資料（`tasukaru_ryokin_kumikae_shanai.pdf`）は**消した**。
案Aで決まったので、両論併記が残っていると後から見た人が迷う。

## 5. まだ残っていること

- 開発者MENUの `except: pass`（上記）
- 解約後の読み取り専用モードと保管の課金（月額の10%／5年一括のプランも作る）
- 書き出しのPDF版（e-文書法の見読性）
- `invoice.payment_failed` のハンドラが無い
- `PLAN_ENFORCE = True` の前に、資料に書いてあって門が無い機能をそろえる

# 2026-09-11（金） — 課金まわりを頭から洗い直した日  <!-- readme-20260911 -->

| 本番のコミット | 中身 |
|---|---|
| `d806b10` | `onboard-lump-trial-v1`（＋前日のREADME） |
| `c0a35b1` | `stripe-switch-cancel-v1` / `v2`、`stripe-api-version-v1` |
| `dfda89e` | `plan-contract-guard-v1`、`soge-bar-safearea-v1` |
| `c6b3131` | `plan-trial-carry-v1` |

朝の「無料体験は一括払いの人だとなぜできないの？」という質問から始まって、
決済の動線を全部たどり直した1日。★出てきたのは全部、
**最初の有料のお客様が付いてから気づいたら手遅れ**のものだった。
いまは自社1施設しか使っていないので、実害はゼロで間に合った。

## 1. ★いちばん重い — 払っている施設が締め出される（`stripe-api-version-v1`）

`requirements.txt` は `stripe==15.2.0`。このライブラリが使うAPIの版は
**2026-05-27（dahlia）**。この版で、次の2つが【置き場所ごと消えていた】。

| app.py が見ていた場所 | いまの場所 |
|---|---|
| `subscription.current_period_end` | **`subscription["items"]["data"][].current_period_end`** |
| `invoice.subscription` | **`invoice.parent.subscription_details.subscription`** |

結果、`_stripe_sub_expiry()` が**いつも None を返し**、`invoice.paid` も
どのサブスクの入金か特定できず、**`expires_at` が一度も延びない**。
ログインは `expires_at` で弾くので、こうなる。

    LIFFから月払いで申し込んだ施設 … 申込から【63日目】にログイン不可
                                      （無料60日 + 猶予3日）
    /pricing から月払いで契約した施設 … 契約から【33日目】にログイン不可
    一括払い                        … expires_at = 契約満了日なので影響なし

**カードは毎月引き落とされ続ける。払っているのに入れない。**
二重課金より始末が悪い。相手は正しく払っているのに、こちらが締め出している。

★**ライブラリの版を上げたら、使っている値が生きているか確かめること。**
インストールして `Subscription` と `Invoice` の中身を見れば数分で分かった。
「前は動いていた」は、SaaSのAPIでは根拠にならない。

★`sub["items"]` と**キーで引く**。`sub.items` は辞書のメソッド名とぶつかるので、
ライブラリの版に左右される。キーなら必ず中身が取れる。

★取り出せなかったときに**黙らせない**。ログとLINEで知らせるようにした。
この不具合が2ヶ月間ずっと気づかれなかったのは、
`if cpe:` が静かに素通りしていたから。

ついでに `invoice.payment_failed` の受け口も足した（**知らせるだけ・DBは変えない**）。
カードの再試行中に現場の手を止めない。払われないままなら猶予のあとで自然に締まる。

## 2. 乗り換えで二重課金（`stripe-switch-cancel-v1` / `v2`）

Stripe Checkout は subscription モードで走るたびに**新しいサブスクを作る**。
古いほうを止めていなかった。

    月払い → 一括     : 一括の全額を払った上に、毎月の請求が続く。
                        しかも stripe_subscription_id が None で上書きされ、
                        古いIDを二度と辿れなくなる
    プラン変更(月払い) : 新旧2本のサブスクが並んで両方から請求が出る
                        ★こちらのほうが起きやすい

**直し方**

- 施設の行から【今の】サブスクIDを先に読む
- 新しいIDと違えば、古いほうを `cancel_at_period_end=True` で止める
- ★**即時解約にしない。**支払い済みの当月ぶんを取り上げないため。
  無料期間中の乗り換えなら、そのまま無料で終わって課金は一度も起きない
- ★**止められたときだけ**列を書き換える。止められなければ古いIDを残して
  LINEで知らせる。消すと見失う
- ★**Webhookが二重に届いても、今作ったサブスクを解約しない。**
  `古いID != 新しいID` のときだけ止める。ここを外すと正常な契約を自分で殺す

**v2 で塞いだ穴。** 解約したあとに再契約する施設は、古いサブスクがもう
`canceled` なので `modify` が失敗する。v1 はそれを「止められなかった」と読み、
**毎回まちがったLINE通知**を出して、止まっているIDを列に残していた。
★**通知が狼少年になると、本物を見落とす。**止める前に `status` を見るようにした。

★あわせて `customer.subscription.deleted` に歯止めを入れた。
あれはサブスクIDが一致する施設を問答無用で `is_active=False` にする。
v1 で「止められなかったらIDを残す」ようにしたので、そのIDを後から手で解約すると
**3年ぶん前払いした施設が締め出される**。前払い(lump)の施設は無効化しない。
契約種別が読めないときも無効化しない。
★**使わせ続けるのは取り返しがつくが、締め出しは取り返しがつかない。**

## 3. 一括なのに「2ヶ月無料」と出ていた

**Stripeの仕組みの話。** `trial_period_days` は `subscription` にしか無い。
一括は `mode="payment"`（一回きりの支払い）なので、無料期間を付ける手段が
そもそも無い。これ自体は正しい。問題は**画面がそう書いていなかった**こと。

**LIFFの申込フォーム**（`onboard-lump-trial-v1`）
画面の上に緑の帯で「2ヶ月無料」が**常に**出ていて、一括を選んでも消えない。
無料にならないと書いてあるのは、注意書きの**最後のカッコの中だけ**だった。
その状態で3年一括を選ぶと、その場で302,400円が決済され、返金もない。
★カッコ書きを見落とした人に「無料期間中に決めればいいと思った」と言われたら、
**こちらに分が悪い。有利誤認になりかねない。**
→ 一括を選んだときだけ帯を消し、赤い注意書きを出し、
**「上記を確認しました」にチェックが入るまでボタンを押せなくした。**

**`/pricing`**（`plan-trial-carry-v1`）
こちらはもっと広かった。「2ヶ月間は無料でお試しいただけます」が
**誰にでも無条件で**出ていたが、`create_checkout` は `subscription_data` を
渡していないので、**この画面からの契約に無料期間は一度も付かない**。
2ヶ月無料が付くのは LIFF の新規申込だけ。**全部の契約期間で嘘をついていた。**

さらに、**無料期間の途中**でプランを変えると、新しいサブスクが無料期間なしで
作られ、その場で課金が始まる。残っていた無料の日数が消える。
★「無料のうちに試して決める」が成り立っていなかった。
→ `facilities.trial_ends_at` を `subscription_data.trial_end` として渡し、
**残りの無料期間を引き継ぐ**ようにした。

★**Stripeは `trial_end` を【48時間以上先】しか受け付けない**（ライブラリの
説明に明記）。余裕を見て49時間より先のときだけ渡す。足りなければ渡さず、
画面にも「すぐに課金が始まります」と出す。**言えないことを言わない。**

画面の文言は3通りに分けた。

    残り3日以上 … 残りの無料期間はそのまま引き継がれます
    残りわずか  … 無料期間はまもなく終わります。すぐに課金が始まります
    無料期間なし … お申し込みの手続きが終わると、すぐに課金が始まります

## 4. 契約に鍵が無かった／割引が読めないと満額（`plan-contract-guard-v1`）

**解約**（`/api/cancel_subscription`）は `admin_authenticated` を要求していたのに、
**契約**（`/api/stripe/create_checkout`）は**ログインしていれば誰でも**叩けた。
★**向きが逆。お金が出ていくほうに鍵が無い。**
一般職員がプロ3年一括（302,400円）を通せる状態だった。解約と同じ鍵・同じ文言にそろえた。

★LIFFの新規申込には鍵をかけない。**ログイン前なので、かけると誰も申し込めなくなる。**
パッチの中にその点検を入れてある。

**割引**は、次の3つのどれが起きても【黙って満額】で決済に進んでいた。

- `facilities` から割引を読めなかった（DBの不調）
- 割引率は入っているのに、対応するクーポンの環境変数が空
- 想定していない割引率が入っていた

50%引きを約束したお客様から満額を取ることになる。
★**止まるほうを選ぶ。返金と謝罪より、売り逃しのほうがまし。**
止めたことはLINEで知らせる（設定の直し忘れにその場で気づけるように）。
割引が設定されていない施設は、これまでどおり普通に契約できる。

## 5. 運行画面の帯が時計の下に潜る（`soge-bar-safearea-v1`）

事業所に着いたとき、「帰着の時刻を直せませんでした（電波が届いていません）」の
赤い帯が iPhone の**時計と電池の下に潜って**出た。文の1行目が読めず、
右端の「閉じる」が電池に隠れて押せない。**消せない帯が画面の上に居座る。**

原因は `base.html` が `viewport-fit=cover` なので、`top:0` の固定要素は
**必ず時計の下に潜る**こと。`base.html` 自身の帯も、同じファイルの `.sr-toast` も
`env(safe-area-inset-top)` で避けていた。★**`.sr-bar` にだけ入っていなかった。**

実機と同じ条件（時計47px）で測って確かめた。

| | 文の位置 | 「閉じる」の位置 |
|---|---|---|
| 直す前 | top=10 ★時計と重なる | top=14〜47 ★丸ごと電池の下 |
| 直したあと | top=59 | top=57〜101 |

★**報告に無い不具合も一緒に出てきた。**本文を空ける余白が `60px` の決め打ちで、
帯の実際の高さが62px。**本文のいちばん上のボタンが2px隠れていた。**
赤と黄の2本が出ると13px隠れる。文が3行に折り返せばもっと。
→ 高さを**測って**決めるようにした（`--sr-bars`）。
★決め打ちの余白は、文が伸びた日に必ず破れる。

なお**時刻を直せなかったこと自体は作りのとおり**。打刻（到着）は電波が無いと
端末に貯めるが、出発と帰着の「直す」は貯めない（その便に1つしかない値なので、
あとから勝手に上書きしないため）。事業所は携帯回線からWi-Fiへ切り替わる場所なので、
通信が一瞬切れやすい。**帯が読めて消せれば「もう一度押す」で済む。**

## 6. DEVのデモデータ

明日の説明用に、DEMO001へサンプルを足した（`demo_fitness.sql`）。
体力測定と体重を **2026年4月〜9月の毎月1回 × 53名**。
グラフは「直近6ヶ月」を描くので、**過去5か月ぶんが無いと線にならない**。
月を追うごとに握力↑ / CS-30↑ / TUG↓ / 5m歩行↓ / 片脚立位↑ にしてあるので、
「訓練の成果が出ています」という話ができる。書類出力の**データ充足チェック**の
「体力測定」「体重」も埋まる。

★**一意制約を確かめずに書いて1回落ちた**（`body_weights` の
facility_code + patient_id + measured_date）。`on conflict do nothing` を足して、
HIROさんが手で入れた既存の行は飛ばすようにした。6月だけ51件なのはそのため。

送迎で利用者を引っ張ってこられるように、利用日を埋めるSQLも用意した
（`demo_visit_days.sql`）。`patient_visit_days` の `weekdays` は
**曜日の数字をつなげた文字列**（`135` = 月水金）、`ampm_per_day` は
`{"1":"ALL","3":"ALL"}` の形。0=日 … 6=土。`patient_id` は **text** で、
中身は **`patients.id`（整数）を文字列で**。
★ここに最初「`patient_profiles.id` のUUID」と書いた。**間違い。**
UUIDで入れた行は、行はできるのに**送迎表に一生出てこない**（エラーも出ない）。
翌日 `demo_soge_today.sql` で作り直した。詳しくは次の節。

## 7. 今日の作り方について

★**点検の数を頭で数えて、今日も2回外した。**（`applyLumpNotice();` の呼び出し数、
`--sr-bars` の出現数）。どちらも当てる前に落ちたので実害は無い。
そこで数え方を変えた。**足した文字列そのものから数を出す。**

    _ADDED = A_NEW.replace(A_OLD, "") + B_NEW.replace(B_OLD, "") + ...
    assert out.count(MARK) == _ADDED.count(MARK)

これなら、あとで文面を足しても点検が勝手に付いてくる。
★**「4個のはず」と書くのをやめて、「足した中に何個あるか」を数える。**

★**app.py を触るパッチには `compile(out, "app.py", "exec")` を入れる。**
字下げが1つずれるだけで決済のWebhookが丸ごと落ちる。当てる前に止める。

★**「足した以外のところが変わっていない」ことを毎回確かめる。**
足したぶんを取り除いたら元のファイルに戻ること。これが通れば、
うっかり別の行を巻き込んでいない。

## 8. まだ残っていること

- **端末申請の通知が「送っていないのに送ったと出る」。**
  `_sl_notify_admins_device_request` は、管理者のLINEが1件も見つからなくても
  黙って戻る。画面には無条件で「事業所の管理者のLINEにお知らせしました」と出る。
  ★申請した職員は、届いていない知らせを待ち続ける
- 新しいパソコンの端末承認が、本番でも止まっているかどうか未確認
- `_to_plain_dict` が `{}` を返すと、**入金があっても何も起きない**
  （コード内のコメントで自己申告されている。未着手）
- 開発者MENUの `except: pass`
- 解約後の読み取り専用モードと保管の課金（月額の10%／5年一括）
- 書き出しのPDF版（e-文書法の見読性）
- `PLAN_ENFORCE = True` の前に、資料に書いてあって門が無い機能をそろえる
- 出発・帰着の「直す」を、電波が無いときに貯めるかどうか（いまは貯めない）


# 2026-09-11（金）午後〜12（土） — 「いつからそうだったか」を持っていなかった日  <!-- readme-20260912 -->

今日いちばん大きかったのは、**同じ形の穴が2か所にあった**こと。
どちらも「今の値しか持っていないのに、過去の帳票をその値で作っていた」。

    利用曜日   patient_visit_days   … 今の設定しかない → 先月の予定の見え方まで変わる
    勤怠設定   timecard_config      … 今の値しかない  → 先月の給与出力が新しい割増率で計算し直される

自費（`patient_jihi_weekdays`）だけは前から `valid_from` / `valid_to` を持っていた。
**そちらに合わせて、両方を有効期間つきにした。**

## 1. 利用曜日の有効期間（`visit-weekday-history-v1` / `visit-weekday-applyfrom-v1`）

きっかけはHIROさんの質問。
「曜日のトグルって月途中でその曜日の利用がなくなり消した場合には
その前までのものは特に問題ない？」

**問題ある。** 火曜をやめると、先月のカレンダーからも火曜が消えていた。
実績（`visit_records`）は残るが、**予定**は今の設定から毎回描き直していたため。

### 表の形

    patient_visit_weekdays (facility_code, patient_id, weekday, state, nth,
                            valid_from, valid_to)

- **`patient_visit_days` はそのまま残す。** あちらが「今の設定」、こちらが「いつからの分」。
  送迎はあちらを見ているので、触ると当日の運行に響く。
- `patient_id` は **`patients.id`（整数）を文字列で**。自費はUUIDだが、
  こちらは `patient_visit_days` の履歴なので**鍵も同じものにそろえる**。
- **`valid_to` はその日を含まない。** 3月1日にやめたなら `valid_to = '2026-03-01'`、
  2月28日までが有効。自費の `jihi_active_on` と同じ約束にした。
- `state` に **`UNSET` を足した。** 曜日には入っているが午前/午後/終日が決まっていない
  旧データがある。**落とすとその人の予定が履歴から消える。**
  移行のSQLで `AM/PM/ALL` 以外を `UNSET` にして拾った。

### 移行

今の設定を **1900-01-01 から有効**として1本入れるだけ。
★**過去を推測して作らない。** 「この頃は火曜に来ていたはず」を実績から逆算すると、
推測を記録として残すことになる。監査で説明できない。
結果：本番 126行/86人、DEV 120行/53人。曜日別の人数と一致。

### 適用開始日（第4段階）

自費にあった「適用開始日」の箱を、**保険の曜日・自費・第N週の3つ共通**に広げた。
日付の箱を2つ並べない。同じ画面に似た日付欄が2つあると、どちらを触ったのか
分からなくなる（この日、まさにそれで事故が起きている。→ 3節）。

    今日から / 今月1日から / 日付を指定（今日まで） / 過去にさかのぼって全部変更

★**未来の日付は受けない。**
「来月から火曜をやめる」を許すと、`patient_visit_days` は**その場で**変わるので
**送迎は明日から乗せなくなる**のに、履歴は来月まで火曜のまま。
食い違ったまま人が動くと、送迎に穴があく。先の予約は別の仕組みが要る。
画面は `max=今日`、サーバー側でも今日に丸めてログに残す。

### DEVで実測（9月11日に火曜をやめて、すぐ戻す）

    8月   4火 11火 18火 25火 …… そのまま残る
    9月   1火 8火 だけ残る（11日より前）
    10月  火なし

## 2. ★木曜が消えなかった件 — 自費を見ずに2回まわした

上の検証中、**10月に木曜が出続けた**。今の設定（火土）でも履歴（火土）でも
出ないはずで、どちらとも合わない。切り分けの実験まで組んだ。

答えは**その人が木曜に自費だった**だけ。`/api/visit/month` は

    if is_jihi:  planned = True; visit_type = 'jihi'      # ★自費が先
    elif _visit_is_planned_weekday(...):  visit_type = 'full' / 'half'

の順で見るので、保険の木曜を消しても自費として出続ける。
**`visit_type` を見れば一発で分かった**（返り値に入っている）。
★**手元に答えが返ってきているのに、仮説から入った。**
おかしいと思ったら、まず**返ってきている値を全部見る**。

## 3. 勤怠設定の有効期間（`timecard-config-history-v1`）

こちらは**もっと悪い形**だった。

### ★設定画面に「サービス提供時間」という見出しが2つある

    サービス提供時間（参考）      service_times        … どこにも使われない  ← 上
    型ごとのサービス提供時間      *_service_slots      … ★【勤務予定】の様式はこちらを使う ← 下

2026-09-01 から午後の2単位目を 13:00-16:00 → 13:30-16:30 に変えた、その変更が
**上だけに入っていた。** 下は 13:00-16:00 のまま。
つまり**変えたつもりで、様式4には反映されていなかった。**

### ★訂正 — 提供時間は「実績」の様式には効かない  <!-- readme-20260913-fix -->

最初この節に「時間を変えると**先月の様式4が新しい時間で出る**」と書いた。
**出した実物で確かめたら、違った。**

- **実績**の様式4（`kinmu_jisseki_*.xlsx`）は、実際の打刻（`timecard_records`）から作る。
  サービス提供時間は**一切使わない**。
- 提供時間から打刻を合成するのは **`is_yotei`（勤務予定）のときだけ**。
- しかも様式に書くのは時刻ではなく【**時間数**】。
  `13:00-16:00` も `13:30-16:30` も 3時間。**数字は変わらない**
  （半日型は2単位で6時間、`half_slot_hours=4` で頭打ちなので、どちらも「4」）。

★**帳票の中身を見ずに、コードの見た目から危険を見積もった。**
`cfg.get("half_service_slots")` が様式の関数の中にあったので、
「様式が時間を使っている」と決めつけた。**その行は `if is_yotei:` の中**だった。

では履歴が要らなかったかというと、そうではない。**効く先が違っただけ。**
同じ設定の中に、給与に直接効く値が入っている。

    締日 / 時間外・深夜・法定休日の割増率 / 1日・週の法定労働時間 / 週の起算日
      → _pay_compute_staff(days, cfg) と _pay_period_range_jst(..., closing_day)

これらを今日変えると、**先月・先々月の給与出力が黙って計算し直される。**
社労士に出したものと数字が変わる。そちらが本当の効き目。
提供時間のほうも、次に「長さ」が変わる改正のときには数字が動く。

★**似た名前の箱を2つ置いて、効かないほうを上に置いた**のがそもそもの罪。
見出しを直した。

    サービス提供時間（表示用）        ★ここは様式には使われません
    型ごとのサービス提供時間（★様式に出るのはこちら）

### 履歴の形

    timecard_config_history (facility_code, valid_from, value jsonb, created_by)

- **`valid_to` を持たない。** 「その日以前でいちばん新しい行」がその日の設定。
  利用曜日は曜日ごとに独立して開閉するので `valid_to` が要ったが、
  こちらは設定まるごと1本なので開始日だけで足りる。
- 設定を**まるごと**入れる。どのキーが変わったかを覚えない。
  「あとで新しい設定項目が増えたとき」に効いてくる。
- **同じ施設・同じ日に2本置かない**（一意索引＋アプリ側で差し替え）。

### 読む側（7か所）

    admin_timecard_youshiki      … ★その【日】。月の途中の変更も拾う
    admin_timecard_monthly       … その月
    pay_export_simple_csv/excel  … その月
    pay_export_payroll_csv/excel/pdf … その月

★**履歴が読めない・1本も無いときは今の設定を返す。空を返さない。**
「読めなかった」を「設定が無かった」にしない。

### 種まき

履歴が1本も無い施設では、**最初の保存のときに「変更前の設定」を 1900-01-01 で記録**する。
★これを**書き換えの前**に呼ぶ。あとでは変更前がもう分からない。
利用曜日の `_vw_seed` と同じ形にそろえた。

## 4. 入金の消し込み（`dev-billing-v1`）

開発者MENUに「入金の消し込み」を作った。
施設が増えると、通帳にはまとまった金額しか乗ってこない。

- Stripeの**振込（Payout）ごと**に、中身（どの施設のいくらが入っているか）を開く
- **施設ごと**のタブでも見られる
- 返金は元の請求から施設を引いて**マイナスで並べる**。振込手数料は「（振込手数料）」
- 合計と振込額の**ずれ**を出す。ずれていたら赤

★読み込んだ結果を `case-photos/billing/<年月>/<取得日時>.json` に**版として保存**する。
次に開くときはそれを読む。保存を兼ねた作りになっている（上書きしない。版を足す）。
CSV（utf-8-sig）とPDF（pdfkit・横向き）で出せる。

★**PDFのCSSに `width:100%` を書いて `%` 書式と衝突し、必ず落ちる状態だった。**
文字列連結に変えた。`make_response` は関数の中で import する（app.py の先頭に無い）。

## 5. 点検のやり方を変えた ── 字面ではなく構造で数える

今日だけで**4回、点検の数を頭で数えて外した。**
とどめが `visit-days-id-fix-v1` の**塞ぎ漏らし**。
利用曜日を書く入り口を4つ塞いだつもりが、**5つ目**（`/api/save_weekday_nth`）が
26642行という離れた場所にあって当たらなかった。
★**近くにあるものだけ見て「4つ」と決めた**のが間違い。

v2 で数え方を変えた。**app.py を構文として読んで、条件に当てはまる関数を全部出す。**

    _tree = ast.parse(out)
    _lines = out.split(chr(10))          # ★行番号で切り出す
    for _n in ast.walk(_tree):
        _seg = chr(10).join(_lines[_n.lineno-1:(_n.end_lineno or _n.lineno)])
        if 'table("patient_visit_days")' not in _seg: continue
        if ".insert(" not in _seg and ".update(" not in _seg: continue
        ...

★`ast.get_source_segment` は使わない。**4万行では終わらない**（実際に2分で止まった）。
行番号で切り出す。

★**通してよい関数は、名前だけで許さない。その根拠が中に在ることも確かめる。**

    _OK_WITHOUT = {"api_bulk_register_patients": "_patients_insert_guarded"}
    assert _why in _body, "★%s を通す理由が中に見当たりません" % _f

勤怠設定でも同じやり方を使い、**設定を読む10関数のうち7つが
「その月の設定」で読んでいること**を、当てる前に確かめた。

## 6. その他

- **11名以上の月額が違約金に入っていなかった**（`plan-penalty-staff-v1`）。
  40名・プロ・3年で 92,000 → 158,000円。`staff_limit` は**別クエリ**で読む。
- **モニター施設の歯止めが、他のJSエラーで丸ごと外れた**（テストで再現）。
  大きな `<script>` の中に書いていたため。**カード直後の独立した `<script>` に移した。**
  `DOMContentLoaded` も待たない。★**歯止めは、壊れやすいものと同じ籠に入れない。**
- **運行画面の帯が時計の下に潜っていた**（`soge-bar-safearea-v1`）。
  `viewport-fit=cover` なので `top:0` は**ステータスバーの下**。
  `env(safe-area-inset-top)` を足す。本文の余白も2px足りずボタンが隠れていた。
- **本番の `patient_visit_days` に壊れた行が10件**あった。全員に正しい行が別にあったので
  幽霊行と判明。ガード付きDELETEで除去して86件すべて正常に。

## 7. まだ残っていること

- 本番の10名（尾前京子・長松軒茂子・安藤香・松下時子・小泉淳子・小倉愈・古橋徳子・
  日高美保子・杉浦信行）の**午前/午後の設定を目で確認**する
- 本番 `/dev/billing` を実際の振込月で開いて、**通帳と一致するか**確かめる
- `_to_plain_dict` が `{}` を返すと、**入金があっても何も起きない**
- 開発者MENUの `except: pass`
- 解約後の読み取り専用モードと保管の課金
- `PLAN_ENFORCE = True` の前に、資料に書いてあって門が無い機能をそろえる
- 出発・帰着の「直す」を、電波が無いときに貯めるかどうか（いまは貯めない）
- 端末申請の通知（`_sl_notify_admins_device_request`）は直したが、本番で未確認

# 2026-09-13（日）〜14（月） — 同じものが何か所にもある、を6件見つけた日  <!-- readme-20260914 -->

## 1. ★この2日の主題

**同じ決まりが、離れた何か所にも書いてある。** 片方だけ直して食い違う。
今週これを **6件** 見つけた。うち1件は自分で踏んだ。

| どこ | いくつ | どうなっていたか |
|---|---|---|
| サービス提供時間 | 2箱 | 表示用と型ごとで食い違う |
| 利用曜日を書く入り口 | 5か所 | 直しても届かない経路が残る |
| 記録のカテゴリ | 6+3か所 | 足したカテゴリが書類に出ない |
| 配車の優先順位 | — | どちらが勝つか画面のどこにも書いていない |
| 画面ごとの見出し（評価） | 3か所 | 入口を足したら名前も月も出なかった（自分で踏んだ） |
| 意味のない文字の一覧 | 1つに | 6件目。**はじめから1つにした** |

★**「あとで揃える」は来ない。** 気づいた回で1つにする。

---

## 2. ★配車が効かなくなった（9/14・現場から緊急）

### 起きたこと

久しぶりに来る利用者を **いつもの配車** に入れたのに、運行画面に出ない。
「配車表を取り込む」を押しても変わらない。

### ★原因を2回外した

1回目「中止フラグが残っている」→ 違う。
2回目「市川さんが落ちている」→ 「市川さんは今日休みだからこれでいい」。

**名前を3回聞いて、やっと藤本さんの中間便の送りだと分かった。**
配車表ではセレナ、運行表ではキャラバン。

★**手元に突き合わせられる表が2つあるのに、仮説から入った。**
先に運行表と配車表を並べて差を数えていれば、1回で着いた。

### 本当の原因 — 優先順位がどこにも書いていない

    _soge_week_for_day の順
      ① _soge_saved_date … 今日だけの配車（soge_date_routes）
      ② _soge_saved_week … いつもの配車（soge_routes・曜日）
      ③ soge_build_week  … その場で組む

★**その日に soge_date_routes があると、いつもの配車は一切見ない。**
直しても、取り込みを押しても変わらない。エラーも出ない。

★「今日だけの配車」は **その画面で保存を押したときにしか作られない**。
中身を変えなくても作られる。だから身に覚えがないまま出来ている。

### 直したこと

- `soge-dateplan-notice-v1` 運行画面の帯に出す＋［いつもの配車に戻す］
- `soge-dateplan-notice-v2` ★v1は **一度も出なかった**。
  この画面はサーバーの戻りを `state.data` に入れるのに、`state` から直接読んでいた。
  `locked` / `past` も同じ間違い。**DEVに実際に作って開いたから見つかった。**
- `soge-dateplan-list-v3` **いつもの配車の画面にも**出す。
  曜日タブの下に、その曜日の「今日だけの配車」がある日を並べ、日ごとに戻せる。
  ★**直している人は運行画面にいない。この画面にいる。**

### ★取り消した案内

恒久的に配車を変えたい人に「今日だけの配車で設定してください」と案内した。
HIROさんに「おかしいよね？」と指摘されて撤回。
それをやると、その日限りの設定が毎週増える。
正しくは **いつもの配車を直す →［配車表を取り込む］**。
その日の「今日だけの配車」を削除すれば、削除処理が当日の運行表まで差分で反映する。

---

## 3. 記録カテゴリの出どころを1つに（`rec-cat-single-source-v1` / `v2`）

管理者MENUで「看護」を足しても、**モニタリングにも書類にも出てこなかった**。
記録は入る。エラーも出ない。書いた人は載っているつもりでいる。**これが一番まずい形。**

同じ一覧が **サーバー6か所＋画面3か所** にベタ書きされていた。

    _rec_categories(supabase, f_code)         出どころ。読めない/0件なら従来の一覧に倒す
    _rec_categories_valid(supabase, f_code)   保存を許す側。★広げるだけ。狭めない

★**空を返さない。** 空＝「カテゴリが1つも無い」で、書類が丸ごと空になる。
★**登録に無い古いカテゴリも後ろに足す。** 消したカテゴリで書かれた過去の記録を落とさないため。
★**狭めない。** 狭めると、いま使っているカテゴリが弾かれて保存できなくなる。

点検は字面ではなく構造で。`ast` で全関数を走査し、
「ヒヤリハット」を含む関数が **0** であることを確かめた（残るのは受け皿の1か所のみ）。

---

## 4. 職員のふりがな／職員情報を1枚に

- `staff_kana` 列を追加（`staffs`）。数秘の **D・S・P・M・IT・LL は名前のローマ字から出す**ので、
  ふりがなが無いと8つのうち2つ（LP・B）しか出なかった。利用者は前からあり、職員だけ欠けていた。
- ★列が無い環境で職員が消えないよう、**読み直す**受け皿を付けた。
- `staff-info-panel-v1` 誕生日・ふりがな・職種・兼務・勤務形態を **1つのボタン・1枚・保存1回** に。
  3つの口へ順に送り、**1つでも失敗したら「保存しました」と言わない**。

---

## 5. 看護の音声語彙（`rec-asr-nurse-v1`）

カテゴリ名に「看護」「医療」「処置」が入るときだけ、看護の言葉を聞き取りのヒントに **足す**。

    まっしょう → 末梢（抹消ではない）   しゅちょう → 腫脹
    ねっかん → 熱感                     とんぷく → 頓服
    そうぶ → 創部                       かくたん → 喀痰 …

★**差し替えない。足すだけ。** 前の版で「差し替える」と書いたが誤り。
看護の記録にも入浴・食事・移乗の話は必ず出る。介護の語彙を外すと
「かいじょ→介助」が看護の記録でだけ効かなくなる。

---

## 6. 評価の入口を3つに（`eval-three-entrances-v1` / `v2`）

評価担当者・**看護師**・機能訓練指導員。

★**ICFの3軸（機能・活動・参加）は個別機能訓練計画書のもの。**
通所介護計画書の援助目標とは別物（HIROさんの指摘で取り違えが判明）。
評価ページの3軸には触らない。

- 看護の欄は `source_data_ns`。ロックも入口ごと（`editing_by_ns`）。
  ★**1つの列を3人で使わない。** 同じ月を2人が開いて保存すると、あとの人で先の人の文章が消える。
- 報告文のAIは **3つとも読む**。読ませなければ入口を増やした意味がない。

### ★v1の抜け（自分で踏んだ）

画面ごとの見出し（名前・対象月・介護区分）が **3か所のベタ書きの一覧** で書かれていて、
画面を足したのに足し忘れ、**看護のページだけ名前も月も出なかった**。
v2で `A2_HEADS = ['a2-who','a2-b','a2-c','a2-d']` の1か所にまとめた。次は1行足すだけ。

---

## 7. 通所介護計画書（`tsusho-keikaku-v1` / `v2` / `print-v1` / `print-v2`）

9/11のプレゼン（まほろばの郷 デイサービス）で必要になったもの。実物の書式を見て作った。

### ★通所介護計画書に全国共通の法定様式は無い

厚労省が出しているのは参考例。**書くべき中身は運営基準で決まっている**が、
様式は事業所ごと。だから**様式に合わせて作り込まない。中身（項目）を持って、出すときに様式を当てる。**
（モニタリング表のほうは、自治体が配っている参考様式そのままの形だった）

### 持ち方

    tsusho_plans              計画書1枚
    tsusho_plan_goals         課題／長期／短期を【番号付きの並び】で1つの表に
    tsusho_plan_services      週間計画表
    tsusho_plan_programs      プログラムの行
    tsusho_program_templates  プログラムの型（施設ごと）
    tsusho_monitorings        モニタリング表の①〜④

★**目標は番号付きの並び。** 実物は課題①〜④と長期①〜④と短期①〜④が番号で対応していた。
数がそろわなくてよい（課題4つ・短期3つ でも成り立つ）。
★**計画書に「終わりの日」を持たない。** その日以前でいちばん新しいものが、その日の計画書。
勤怠設定の履歴と同じ考え方。終わりの日を持つと、閉じ忘れた日にどちらが有効か分からなくなる。
★**子の行は保存のたびに入れ替える。** 行を足し引きすると番号が振り直されるので、
差分では「どの行が同じ行か」を決められない。
★**トグルは表を増やさない。** `admin_settings` の key/value（請求額計算と同じ）。

### 印刷

★**印刷の画面に `base.html` を被せない。**
`base.html` は `body{height:100vh; overflow:hidden}` を入れてあるので、
被せると **印刷が画面1枚分で切れる**。モニタリングは印刷のときだけ打ち消しているが、
打ち消し忘れがそのまま事故になる。もとから被せない（書類出力・送迎の記録表と同じ）。

★**改行は `white-space: pre-wrap` で出す。** `<br>` に置き換えて `|safe` を通すと、
利用者の書いた文字がそのままHTMLとして通る道ができる。留意点には長文が入る。

★**日付は和暦。** 実物が和暦で、ケアマネや家族が他の書類と並べて見る。

★**［印刷］は保存前に出さない。** 紙に出るのは保存された中身。
書きかけがあるときは押す前に言う。**出してから気づくのでは遅い。**

### ★v2で直したこと

消した計画書を開くと「いま開けませんでした。少し時間をおいて」と出た。
消えているので待っても開かない。**「無かった」と「読めなかった」を同じ `None` にしていた。**
送迎（`_soge_date_plan_row`）では分けてあるのに、ここで揃えていなかった。

---

## 8. 数秘が漫画数秘ラボと食い違った（`numerology-hepburn-v1`）

加藤鮎美さん（昭和59年11月28日）で突き合わせたら **D・S・M・IT の4つ** が違った。

### ★分かれ方が原因を指していた

合っていたのは LP・**P**・B・LL。合わなかったのは D・S・M・IT。
**P は子音しか足さない。S は母音を足す。** つまり **母音が1つ多い**。

    TASUKARU   かとうあゆみ → KATOUAYUMI
    数秘ラボ   かとうあゆみ → KATOAYUMI

数秘ラボの計算方法のページに明記されていた。
**「ヘボン式。長音の『う』は書かない（佐藤=SATO）」**

- お段/う段のうしろの「う」を落とす
- 「おお」は語中なら詰める（大野=ONO）、語末は残す（高遠=TAKATOO）

IT（名前に最も多く出る数字）は **同数が並ぶときに全部出す**。
`find()` で最初の1つだけ拾っていた。LLは最初から並びで出していたのに、ITだけ揃っていなかった。

### ★自動では取り切れないものがある

    かとう   → KATO      ○
    いのうえ → INOE      ×（正しくは INOUE）
    まつうら → MATSURA   ×（正しくは MATSUURA）

**長音かどうかは言葉の切れ目が分からないと決められない。**
日本語の側だけを見て正しく決める方法は無い。数秘ラボも人が打つ前提。
なので **ローマ字を画面で直せるようにした**。自動変換は下敷き。
★**この2つの名前を画面に名指しで書いた。黙って外すのがいちばん困る。**

---

## 9. 「文字としての None」（`none-string-guard-v1`）

計画書を印刷したら、住所が **`〒None None`** と出た。
`null` ではなく **N-o-n-e の4文字** が入っていた。
...
★**書類に「担当 None 様」と印字される。渡す相手はそのケアマネ本人。**

### 入口

`api_admin_patient_bulk_import`（CSV一括取込）が、
施設コードと利用者番号以外を **素通し** していた。
他のソフトから出したCSVに "None" が入っていれば、そのまま入る。

★**データを洗うだけでは、次の取り込みでまた入る。入口を塞ぐ。**

### ★自分の決めを1つ引っ込めた

`print-v2` で「空にする文字」の一覧に `-` を入れた。**行き過ぎだった。**
「-」は人が「なし」のつもりで書く字。消すと書いた人の意図まで消える。外した。
そして一覧を **1か所（`_JUNK_TEXT`）** にした。今週6件目なので、はじめから1つにした。

判定は **その欄がその文字だけのとき**。含むかでは見ない（Noneville を壊さないため）。

---

## 10. ★この2日で繰り返した失敗

- **手元に答えが返ってきているのに、仮説から入った**（配車で2回外した）
- **パッチの自己点検が、自分のコメントや自分の正しいコードに引っかかった**（4回）
  → **点検は緩めない。書き方を変える。**
  「f_code が出てこないこと」のような**否定形の点検は、正しいコードに当たる**。
  見るのは「無いときに何をするか」。
- **作って終わりにしない。** `soge-dateplan-notice-v1` は
  DEVに実際に作って開いたから、一度も出ていないと分かった。
- **device_commit_files が古い版を書いた**ことが複数回。
  パッチを置いたら必ずステージし直して **md5 で一致を確かめる**。

---

## 11. まだ残っていること

- 通所介護計画書を **本番へ**（＋本番のトグルをオン）。
  ★数秘の数字が変わるので、「〜とう」「〜こう」の名前の人は驚かれる可能性がある
- 計画書の **写真からの取り込み**（乗り換え用）
- **通所介護モニタリング表**（計画書の目標を見て、AIが下書き→人が確定→月末に一括）
  ★①は【通所介護計画書の目標】に対して答えるもの。個別機能訓練の3軸ではない
- 看護ページの **音声入力**（いまの録音は「聞き取りメモ」1か所に入れる決め打ち）
- 利用曜日・提供時間から計画書の **週間計画表を下敷きにする**
- 数秘のローマ字を **保存できるように**（いまは直しても残らない。列が要る）
- 送迎の記録表で、出発・到着を直そうとすると確定されてしまう件
- 本番 `/dev/billing` の通帳突き合わせ
- `_to_plain_dict` が `{}` を返すと入金が無視される／開発者MENUの `except: pass`
- 解約後の読み取り専用モードと保管の課金／`PLAN_ENFORCE = True` の前に門をそろえる
