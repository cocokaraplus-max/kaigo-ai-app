...
1. **一括読み上げの二重奏バグ（未完全解決）**
   - 時々音声が二重になる
   - 原因：iOS AudioContextのAudioBufferSourceNodeがonended後に予期せず再トリガー
   - 対処済み：`onended = null`してから`stop()`、二重起動防止フラグ
   - **まだ不安定。根本解決が必要**
   - 検討案：fetchが完了する前に次のfetchが始まるレースコンディションの可能性

2. **付箋機能（ブックマーク）**
   - ケース記録でどこまで読んだかを記しておける機能
   - 1つのみ（新しく付けると古いものを外す確認ダイアログ）
   - 次回開いたとき続きから読める
   - `localStorage`で個人スマホのみ発動

3. **新記録追加時のAI要約リアルタイム再生成**
...
## 最新コミット状況
- `tasukaru-dev`: `b7eccd7`（onended clear for double playback fix）
- `tasukaru`（本番）: Session 47の変更がマージ済み

---

**ステータス: 🟡 Session 47完了・一括読み上げ二重奏バグが残存**
