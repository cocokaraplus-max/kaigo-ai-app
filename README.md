...
1. **本番反映**（DDL2つ → merge → push）。まだDEVのみ
2. 第3段：評価・モニタリングへの反映、印刷
3. 第4段：上の「8.」のとおり `source_data` へ繋ぐ＋満足度など3項目を本人に聞く
...
3. **利用者セルフ評価（タブレット）の本番反映。** ★大きな新機能★
   **第1段・第2段は実装済み。DEVで検証も済んでいる。本番だけまだ。**
