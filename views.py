        ...
        if sel != "(未選択)" and txt:
            try:
                ...
                if m:
                    ...
                    supabase.table("records").insert({"facility_code": f_code, "chart_number": m.group(1), "user_name": m.group(2), "staff_name": my_name, "content": txt, "created_at": dt.isoformat(), "image_urls": image_urls if image_urls else None}).execute()
                    st.success("💾 保存完了しました！")
                    time.sleep(1.0)
...
def render_history(supabase, cookie_manager, f_code, my_name):
    ...
    if sel != "---":
        ...
        month_opts = [f"{now_tokyo.year}年{month_num:02d}月" for month_num in range(now_tokyo.month, now_tokyo.month - 6, -1)]
                                    ...
                                    if st.button("🔄 再生成", key=f"regen_{user}", use_container_width=True):
                                        if not normal_recs.empty:
                                            with st.spinner("AIが再生成中です..."):
                                                try:
                                                    recs_text = "\n".join([f"【{r['staff_name']}】{r['content']}" for _, r in normal_recs.iterrows()])
                                                    model = get_generative_model()
                                                    ...
                                                    supabase.table("records").insert({"facility_code": f_code, "chart_number": c_num, "user_name": user, "staff_name": "AI統合記録", "content": summary, "created_at": dt.isoformat()}).execute()
                                                    st.rerun()
                                                except Exception as e:
