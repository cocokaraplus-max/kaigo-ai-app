...
## 35. 【次期大型機能・設計メモ】実績集計表（地域密着型通所介護）（2026-06-26）<!-- readme-s35-session65 -->

SESSION_65末。HIROの帳票「地域密着型通所介護 実績集計表」（まもる君クラウドには出力機能が無く、各報告機関への提出データを手作業で作っている）をTASUKARUで集計・グラフ表示・帳票化したい、という要望。設計を固めて記録。**未着手**。次回ここから第1段階に取り掛かる。

### 35-1. 帳票の集計項目（提出用）
- **介護サービス利用者数・延べ人数**: 要介護1〜5ごとの 実人数 / 割合 / 延べ人数 / 割合、および合計。
- **総合事業サービス利用者数・延べ人数**: 要支援1 / 要支援2 / 事業対象者ごとの 実人数 / 割合 / 延べ人数 / 割合、合計。
- **利用者数合計**: 実人数 / 延べ人数。
- **提供時間別 延べ人数**（地域密着型）: 2-3h / 3-4h / 4-5h / … / 13-14h の区分別。
- **提供時間別 延べ人数**（総合事業）: 5h未満 / 5-7h / 7h以上。
- 注記「実人数は算定上の人数、()内は月末時点での要介護度で計算」。→ **月末時点の介護度**で集計する点が重要。
- HIRO施設例（令06年11月）: 実人数78、延べ407。平日=3-4時間、日曜=7-8時間。

### 35-2. 現状データの可否
- **出せる**: visit_records（来所実績 patient_id/visit_date/status/source、バイタル保存で自動記録 visit-mgmt-v1, app.py 2713付近）＋ care_level（介護度）で、介護度別の実人数・延べ人数・割合は算出可能。月集計 visit_month は app.py 3146付近に既存。
- **出せない（データ不足）**: 提供時間（service_time系のカラム/設定は存在しない＝grep空）。提供時間別集計には新たにデータの持ち方が必要。

### 35-3. 設計方針（HIROと合意）
- **介護度は履歴で持つ**: 現状 patient_profiles.care_level は最新1個のみ。これを「介護度＋変更日（適用開始日）」の履歴テーブル（例 `care_level_history`: id, facility_code, patient_id, care_level, valid_from, created_at）に拡張。利用者情報で介護度変更時に**変更日を入力**し、変更前・変更後の両方を記録に残す。集計時は「対象月の月末時点で有効な介護度」を履歴から引く（過去月も正確に再現できる）。既存 care_level は最新レコードとして移行。
- **提供時間は施設設定で持つ**: ハードコードせず施設ごとに設定。御施設は平日=3-4時間/日曜=7-8時間だが、他施設は異なるため、施設×曜日→提供時間区分のマッピング設定（例 `service_time_settings`: facility_code, weekday, time_category）。集計時は visit_records.visit_date の曜日から区分を導出。将来「同一曜日でも利用者/日ごとに提供時間が違う」施設のため、利用者別・日別上書きの拡張余地を残す。
- **グラフ表示**: 集計した数字をグラフ（介護度別構成、提供時間別など）でも表示する（Chart.js。既存のモニタリング/連絡帳でChart.js実績あり）。提出用の表＋視覚化。

### 35-4. 実装段階（次回これに沿って進める）
1. **第1段階（土台）**: `care_level_history` テーブル作成（DDL先行）。利用者情報（patient_profile）に介護度変更UI（変更日入力・履歴保持）。既存 care_level の移行。
2. **第2段階**: 対象月選択→介護度別の実人数・延べ人数・割合を画面集計（月末時点の介護度を履歴から判定）。提供時間はまだ。
3. **第3段階**: 施設の曜日→提供時間設定（`service_time_settings`）を作り、提供時間別集計を追加。
4. **第4段階**: グラフ表示（Chart.js）。
5. **第5段階**: 提出帳票としての印刷出力（様式再現）。

### 35-5. 注意・論点
- 報告書＝提出物なので数字の正確性が最重要。土台（介護度履歴）を慎重に設計・テストする。
- 月末時点の介護度判定: valid_from を遡り、対象月末日時点で最新の有効レコードを採用。
- 提供時間が曜日一律で全施設をカバーできるかは要検証（御施設はOK）。祝日・臨時営業日の扱いも論点。
- 既存の visit_records が実績の真実источник。バイタル未入力日の扱い（来所したがバイタル無し等）が集計に影響しないか確認。
